...
| Temporal validation | `INVALID_PM_QUALITY_BUSINESS_DATE:<field>`, `INVALID_PM_QUALITY_UTC_TIMESTAMP:<field>`, or `INVALID_AS_OF_DATE` | Correct malformed date-only fields to `YYYY-MM-DD` and timestamp fields to timezone-aware instants. Do not coerce timestamps into business dates or infer dates from month labels. |
...
- For portfolio-scoped downstream evidence, the same endpoint must also return
  ...
  unsupported tenant assertion. Manage preserves source-owned Risk/Performance mandate-health
