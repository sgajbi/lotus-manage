...
- In production profile, readiness also validates trusted write authorization posture, bounded
  Postgres access policy, and required cutover migrations before reporting ready.
...
- Metrics labels must remain bounded and must not expose portfolio ids, client names, account ids,
  ...
  payloads.
- Postgres connection metrics use bounded operation/outcome/reason/classification labels and must
  not expose DSNs, SQL text, table names, portfolio ids, request hashes, or payload content.
...
- Missing source evidence or mixed source as-of dates remain degraded/fail-closed; consumers must
  ...
  execution authority. Manage preserves source-owned Risk/Performance mandate-health
  `as_of_date` metadata when supplied by the producer; older, missing, or inconsistent source
...
- Proof-pack report-input and AI-evidence payloads can carry bounded
  ...
  context envelope hash over report-safe event refs with event timestamps and selection ranks plus
  explicit event-ref limit, selection policy, returned-count, omitted-count, and truncation posture
  while excluding that recursive lineage context from the handoff evidence hash.
...
- bounded Manage-owned bulk-review campaign membership through
  ...
  `BulkReviewCampaignDefinition:v1` campaign definitions, retirement/supersession lifecycle
  controls, and
...
- AI evidence input returns bounded source-backed facts, permitted use, forbidden actions, source
  ...
  timestamps and selection ranks, event-ref limit, selection policy, returned-count, omitted-count,
  and truncation posture, and a canonical handoff hash without generating prompts, memos,
...
## Certified endpoint: portfolio memory
...
- composes source events from repositories only and does not synthesize source-owner facts.
...
This endpoint reads manage persistence only. It preserves source refs and hashes emitted by
`lotus-core`, `lotus-risk`, `lotus-performance`, and manage-owned artifacts when those refs are
already present in proof-pack, wave, or outcome-review records.
...
```bash
python -m pytest tests/unit/dpm/api/test_portfolio_memory_api.py tests/unit/dpm/proof_packs/test_proof_pack_repository.py tests/unit/dpm/proof_packs/test_proof_pack_postgres_repository.py -q
```
...
- enabled policies require configured weights, governance approval, and fairness-review evidence,
