...
| DPM command center foundation | `/api/v1/dpm/command-center` | Supported as a bounded Slice 5 read model over persisted monitoring runs and active exceptions. It returns health distribution, attention buckets, recommended actions, and supportability state; Workbench/Gateway integration remains a later handoff. | `src/api/routers/monitoring.py`, `src/api/services/mandate_service.py`, `tests/unit/dpm/api/test_monitoring_api.py`, OpenAPI certification matrix |
...
| DPM command center | RFC-0038 | Bounded command-center summary API is implemented over monitoring runs and active exceptions; PM-book discovery and Workbench/Gateway product-surface integration remain later handoff work. |
