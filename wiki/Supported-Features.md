...
| Explicit portfolio-list waves | Manage owns explicit wave preview/create/source-check/simulate/select/approve/stage/handoff, source-owned PM-book wave discovery, source-owned CIO model-change discovery, bounded source-owned risk-event wave discovery, source-owned risk/performance aggregate analytics posture, report-input, operations handoff evidence, and supportability; Gateway composes the command-center BFF; Workbench renders the first-wave Rebalance Wave Command Center for the canonical portfolio; report/render/archive materialize governed rebalance-wave reports from manage report input; `lotus-ai` owns the guarded wave PM memo workflow pack and owner-side operations handoff summary workflow pack; Gateway and Workbench consume that AI workflow without local prompt or memo generation, while operations-handoff product invocation remains future owner work. | First-wave PM operating cockpit, governed wave report lifecycle, and governed AI PM memo request posture are implementation-backed, CI-proven, live-proven where product-surfaced, and wiki-published. Owner-side operations handoff summary support is now implemented in `lotus-ai` as `dpm_operations_handoff_summary.pack@v1` over bounded Manage `DpmWaveReportInput` handoff evidence with source refs, handoff refs, forbidden-action/output guardrails, review-required support-only output, and no external execution claim. PM-book wave discovery is source-backed through lotus-core `PortfolioManagerBookMembership:v1`; CIO model-change discovery is source-backed through lotus-core `CioModelChangeAffectedCohort:v1`; bounded risk-event wave discovery is source-backed through lotus-risk `RiskEventAffectedCohort:v1` with caller-supplied candidate portfolios and source-supplied exposure weights. Wave aggregate analytics preserve `lotus-risk` and `lotus-performance` supportability, lineage refs, reason codes, and source-emitted scalar values without manage-local risk/performance methodology. Tactical house-view/campaign cohort discovery, Gateway/Workbench operations-handoff invocation, and external OMS execution remain future WTBDs. |
| Report, archive, and evidence materialization | Supported proof-pack, rebalance-wave, and outcome-review evidence paths now emit manage-owned report inputs and AI evidence inputs consumed by `lotus-report`, `lotus-render`, `lotus-archive`, and `lotus-ai` owning services. | RFC37-WTBD-005 is complete for first-wave generated report, archive lifecycle, Gateway/Workbench request posture, AI evidence handoff, report-owned source events, and generated-document/client-delivery source events. `lotus-manage` remains evidence/report-input authority only; client communication execution, external OMS execution, PM scoring, autonomous AI decisions, and new evidence artifact families remain future owner scope. |
| Portfolio memory foundation | Manage composes a deterministic source-backed portfolio-memory read model over mandate health snapshots, monitoring exceptions, proof packs, proof-pack decision timelines, rebalance wave events, internal operations handoffs, and outcome-review events; Gateway composes it for the command center; Workbench renders the first-wave portfolio-memory timeline panel with canonical browser proof. | First-wave PM, operations, audit, report-lineage, AI-support, generated-document/client-delivery archive lineage, and demo-facing event lineage is implementation-backed, source-owned, queryable, live-proven, and wiki-published. Manage now emits stable event identity plus retention, redaction, access, audit policy, bounded `portfolio_memory_context` on proof-pack, rebalance-wave, and outcome-review report inputs, and `source_event_family_posture` naming supported manage/report/AI/archive event families plus explicit `DEFERRED_SOURCE_OWNER` posture for external OMS execution and PM scoring. `lotus-report` now has the bounded context consumer seam through PR #92 and consumes that context without reconstructing manage-owned portfolio-memory events, and `lotus-report` PR #93 exposes report-owned source events for report lifecycle, snapshot, render, and archive evidence. `lotus-ai` has bounded DPM memo/narrative consumers plus an AI-owned workflow-pack source-event family: PR #62 validates the bounded context for DPM PM memo and outcome-review narrative packs without reconstructing timeline facts, and PR #64 exposes the AI-owned no-raw-payload workflow-pack source-event family. `lotus-archive` PR #25 has the generated-document/client-delivery source-event family at `GET /documents/{document_id}/source-events` without raw payload exposure. Future OMS execution and PM-scoring products remain separate source-owner roadmap scope, not hidden portfolio-memory claims. |
...
1. tactical house-view and campaign cohorts are not supported until source owners publish governed
   cohort products and manage consumes them; bounded risk-event waves are supported only through
