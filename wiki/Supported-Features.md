...
| Explicit portfolio-list waves | Manage owns explicit wave preview/create/source-check/simulate/select/approve/stage/handoff supportability; Gateway composes the command-center BFF; Workbench renders the first-wave Rebalance Wave Command Center for the canonical portfolio. | First-wave PM operating cockpit is implementation-backed, CI-proven, live-proven, and wiki-published. Automatic PM-book/CIO/risk-event cohort discovery, owning-service risk/performance enrichment, report materialization, AI memo generation, and external OMS execution remain future WTBDs. |
