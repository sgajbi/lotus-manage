...
tracks 59 WTBD items: 36 done on merged/published truth, 4 partial or in progress, and 19 remaining
...
1. manage risk-event, tactical house-view, and campaign cohorts are not supported until source
   owners publish governed cohort products and manage consumes them; `lotus-risk`
   `RiskEventAffectedCohort:v1` is now available as the first risk-event source product but is not
   yet a manage-supported trigger,
