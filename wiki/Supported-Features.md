...
| Bulk-review campaign membership rebalance waves | `POST /api/v1/rebalance/waves/preview`, `POST /api/v1/rebalance/waves` with `trigger_type=BULK_REVIEW_CAMPAIGN` | Supported as the bounded Manage-owned RFC41-WTBD-003 campaign membership family. Manage requires source-backed candidate portfolios, source-owned `portfolio_type`, source refs, ISO `as_of_date`, and eligible DPM portfolio types; filters out non-eligible portfolio types; emits `BulkReviewCampaignMembership`, `BulkReviewCampaignGovernance`, and `BULK_REVIEW_CAMPAIGN_MEMBER` source refs with deterministic content hash where governance evidence is supplied; preserves underlying source refs from core/risk/performance/advise without recalculating their reasons; optionally validates approval evidence, expiry date, access purpose, source refs, and actor entitlement allow-list; and fails closed for missing candidates, missing portfolio type, missing source refs, invalid dates, empty eligible portfolio-type filters, empty eligible membership, incomplete approval evidence, invalid or expired expiry dates, and unauthorized actors. It does not claim persisted campaign definitions, global portfolio discovery, Gateway/Workbench campaign UI, or OMS execution. | `src/api/routers/waves.py`, `src/api/services/wave_service.py`, `docs/standards/api-vocabulary/lotus-manage-api-vocabulary.v1.json`, `tests/unit/dpm/api/test_waves_api.py`, `docs/rfcs/RFC-0041-rebalance-wave-orchestration-and-cio-model-change-impact.md` |
...
2. PM operating quality scoring is supported only as an explicit Manage-owned score-run lifecycle
   ...
   default, enabled policies require bank-supplied configuration and source-backed evidence,
   optional `pm_book_scope` consumes lotus-core `PortfolioManagerBookMembership:v1` with
   fail-closed source readiness, and portfolio memory publishes `pm_scoring` with
   `SEPARATE_PRODUCT_NO_EVENT_FAMILY` rather than projecting hidden score events,
