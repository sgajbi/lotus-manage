...
| Construction alternative generation | `/api/v1/construction/alternative-sets/generate` | Supported as RFC-0039 Slice 7 manage backend foundation for first-wave methods: do-nothing baseline, explainable heuristic, minimum-turnover, and tax-aware posture. Full product realization through Gateway/Workbench is not yet implemented. | `src/api/routers/construction.py`, `src/api/services/construction_service.py`, `src/core/construction/`, `tests/unit/dpm/api/test_construction_api.py`, OpenAPI certification matrix, `scripts/validate_live_api.py` construction probe |
| Construction alternative read and selection | `GET /api/v1/construction/alternative-sets/{alternative_set_id}`, `POST /api/v1/construction/alternative-sets/{alternative_set_id}/selections` | Supported as persisted backend read and actor-attributed selection foundation. Selection records the preferred alternative but does not execute orders. | `src/core/construction/repository.py`, `src/infrastructure/construction/`, `src/infrastructure/postgres_migrations/dpm/0005_construction_alternatives.sql`, `tests/unit/dpm/construction/test_repository.py`, `tests/unit/dpm/api/test_construction_api.py` |
...
| Advanced construction alternatives | RFC-0039 | Manage backend foundation is partially supported for first-wave generation/read/selection after Slice 7. Full support promotion still requires canonical live proof, hardening, and paired Gateway/Workbench realization RFCs created from manage evidence and then implemented/proven. |
| Tax, liquidity, risk, ESG, currency, and regime-aware construction | RFC-0039 | Proposed. Promote each method only after source supportability, degraded behavior, field-level OpenAPI, and live proof for that dimension are complete. |
