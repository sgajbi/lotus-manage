...
| 2 | RFC42-WTBD-006 - Source-owner realized methodology depth | Promotes aggregate risk, performance, tax, FX, cash, liquidity, and execution methodology from selected adapters into auditable source-owned products. The current performance slices tighten MWR and contribution methodology truth for stateful lotus-core source resolution, carry-forward capital adjustments, source currency metadata, fee exclusion, supportability, and downstream no-reconstruction boundaries. | Owning services provide methodology docs, contracts, degraded-state tests, live proof, and product-surface preservation without manage-local recalculation. |
