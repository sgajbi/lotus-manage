...
| Construction alternative generation | `/api/v1/construction/alternative-sets/generate` | Supported as RFC-0039 manage backend foundation for first-wave methods: do-nothing baseline, explainable heuristic, minimum-turnover, and tax-aware posture. Postgres-backed canonical manage proof passed; full product realization through Gateway/Workbench is not yet implemented. | `src/api/routers/construction.py`, `src/api/services/construction_service.py`, `src/core/construction/`, `tests/unit/dpm/api/test_construction_api.py`, OpenAPI certification matrix, `scripts/validate_live_api.py` construction probe, `output/rfc0039-proof/20260503-173624-canonical-postgres/summary.json` |
...
| Advanced construction alternatives | RFC-0039 | Manage backend foundation is supported for first-wave generation/read/selection after Slice 8 live proof and hardening. Full product-surface promotion still requires the paired Gateway/Workbench realization RFCs to be implemented and proven. |
