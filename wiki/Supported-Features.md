...
| Construction alternative generation | `/api/v1/construction/alternative-sets/generate` | Supported as RFC-0039 manage backend foundation for first-wave and second-wave methods: do-nothing baseline, explainable heuristic, minimum-turnover, tax-aware, solver-constrained, liquidity-aware, risk-aware, ESG-aware, currency-overlay, and regime-stress-aware posture. Second-wave methods preserve degraded source authority instead of fabricating unavailable risk, ESG, FX, or scenario truth. Full product realization through Gateway/Workbench is not yet implemented. | `src/api/routers/construction.py`, `src/api/services/construction_service.py`, `src/core/construction/`, `tests/unit/dpm/api/test_construction_api.py`, OpenAPI certification matrix, `scripts/validate_live_api.py` first-wave and second-wave construction probes, `output/rfc0039-proof/20260503-182819-second-wave-canonical-postgres/summary.json` |
...
| Advanced construction alternatives | RFC-0039 | Manage backend foundation is supported for first-wave and second-wave generation/read/selection after implementation proof and hardening. Full product-surface promotion still requires the paired Gateway/Workbench realization RFCs to be implemented and proven. |
| Tax, liquidity, risk, ESG, currency, and regime-aware construction | RFC-0039 | Supported as manage backend construction postures with explicit degraded-source handling. Promote to full product outcome only after Gateway/Workbench implementation and canonical browser proof. |
