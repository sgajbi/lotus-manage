...
| Mandate digital-twin APIs | `/api/v1/mandates/*` | Supported as RFC-0038 Slice 3 foundation for refresh/read/version/diff. Refresh composes product-specific lotus-core mandate binding, model targets, and optional market-data coverage; source-data gaps remain explicit. Live proof still required before broader command-center promotion. | `src/api/routers/mandates.py`, `src/api/services/mandate_service.py`, `tests/unit/dpm/api/test_mandates_api.py`, OpenAPI certification matrix |
| Mandate health engine foundation | refresh response and internal RFC-0038 foundation | Pure deterministic health scoring across ten dimensions with hard-gate overrides, persistence foundation, and health output during mandate refresh. Standalone health APIs and command-center aggregation remain pending. | `src/core/mandates.py`, `tests/unit/dpm/core/test_mandate_health.py`, `tests/unit/dpm/api/test_mandates_api.py` |
...
| Mandate health score | RFC-0038 | Scoring, persistence, and refresh-response output exists; promote only after standalone health APIs and command-center evidence. |
