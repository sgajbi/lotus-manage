...
| Portfolio memory foundation | Manage composes a deterministic source-backed portfolio-memory read model over proof packs, proof-pack decision timelines, rebalance wave events, internal operations handoffs, and outcome-review events. | Backend authority is implementation-backed for PM, operations, audit, and Gateway consumers that need queryable event lineage. Gateway/Workbench product realization, mandate-monitoring exception nodes, and canonical browser proof remain future slices before full front-office support can be claimed. |
...
tracks 59 WTBD items: 27 done after this slice lands on merged/published truth, 4 partial or in progress, and 28 remaining
...
| ---: | --- | --- | --- |
| 1 | RFC40-WTBD-010 - Decision timeline and portfolio memory | Links mandate, exception, wave, proof-pack, handoff, and outcome evidence into portfolio memory without inventing source truth. Manage backend foundation is now implemented; downstream product realization is next. | Gateway/Workbench event lineage is merged, source-owned, queryable, and live-proven with wiki/demo proof. |
...
| Portfolio memory | `GET /api/v1/rebalance/portfolio-memory/{portfolio_id}` | Supported as a manage backend foundation for source-backed event lineage. The read model composes persisted proof packs, proof-pack-local decision timeline events, RFC-0041 wave events, internal operations handoff refs, and RFC-0042 outcome-review events into a deterministic, hashable portfolio timeline with source systems, source refs, content hashes, supportability state, reason codes, and bounded metadata. It does not compute or reconstruct risk, performance, execution, tax, cash, FX, or source-owner methodology. Gateway/Workbench timeline realization and canonical browser proof remain downstream scope. | `src/core/portfolio_memory/`, `src/api/routers/portfolio_memory.py`, `src/core/proof_packs/repository.py`, `src/infrastructure/proof_packs/`, `tests/unit/dpm/api/test_portfolio_memory_api.py`, `tests/unit/dpm/proof_packs/test_proof_pack_repository.py`, `tests/unit/dpm/proof_packs/test_proof_pack_postgres_repository.py` |
...
| Decision timeline and portfolio memory | RFC-0040, RFC-0041, RFC-0042 | Manage backend portfolio memory is implementation-backed through `/api/v1/rebalance/portfolio-memory/{portfolio_id}` for proof-pack, wave, handoff, and outcome-review lineage. Mandate-monitoring exception nodes and full Gateway/Workbench timeline product realization remain downstream until implemented and canonically proven in the owning apps. |
