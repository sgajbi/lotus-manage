...
| 2 | RFC41-WTBD-003 - Tactical house-view, risk-event, and campaign/bulk-review cohorts | Moves the rebalance wave operating model toward bank operating workflows without inventing source-owned cohorts. Risk-event source ownership and bounded manage consumption are implemented through `lotus-risk` `RiskEventAffectedCohort:v1`. Bounded bulk-review campaign membership with optional governance evidence and immutable campaign definitions over source-backed candidate sets are implemented through Manage-owned `BulkReviewCampaignMembership:v1` and `BulkReviewCampaignDefinition:v1`. Tactical house-view ownership is decided for future `lotus-advise` `TacticalHouseViewAffectedCohort:v1`. | `lotus-advise` publishes the governed tactical house-view cohort before support claims. Global campaign discovery and downstream product surfaces are added only after source behavior and product evidence are proven. |
...
2. PM operating quality scoring is supported only as an explicit Manage-owned score-run lifecycle
   ...
   `PortfolioManagerBookMembership:v1` with fail-closed source readiness, and portfolio memory publishes `pm_scoring` with
