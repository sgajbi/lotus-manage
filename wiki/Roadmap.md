...
| RFC-0041 | CIO and PM teams can orchestrate book-level model-change and rebalance waves. | Implementation is complete through Slice 4: source-map, platform scaffold evidence improvement, cleanup review, wave domain contracts, persistence foundation, explicit affected-portfolio preview, and idempotent durable create. First-wave scope remains explicit portfolio-list waves; automatic PM-book and CIO model-change cohort discovery remain deferred until source-owning products are proven. Support remains proposed until source-check, simulation, approval, handoff, live proof, downstream realization RFCs, and supported-feature promotion are complete. |
