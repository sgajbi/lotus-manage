...
| RFC-0041 | CIO and PM teams can orchestrate book-level model-change and rebalance waves. | Manage backend implementation is `DONE` for explicit portfolio-list waves: source-map, platform scaffold evidence improvement, cleanup review, wave domain contracts, Postgres-backed persistence, preview/create, mixed source-check, RFC-0039-backed ready-item simulation, item-level selection, RFC-0040 proof-pack linkage, approval-with-exceptions, staging, internal operations handoff evidence with no external execution claim, actor-attributed pre-execution cancellation, retrieve/search/item/proof-pack/supportability read models, product-safe diagnostics, OpenAPI certification, hardening review, final gold-pass assessment, and downstream Gateway/Workbench RFC-0098 wave realization addenda. Automatic PM-book and CIO model-change cohort discovery remain deferred until source-owning products are proven. Full product support remains proposed until Gateway and Workbench implementation and canonical front-office proof are complete. |
| RFC-0042 | PMs learn from expected-versus-realized outcomes after execution. | `DONE` for the manage backend authority: Slice 0 source-map guardrails, Slice 1 platform scaffold improvement, Slice 2 cleanup/structure, Slice 3 pure domain comparison, Slice 4 expected snapshot assembly, Slice 5 realized source-degraded handling, Slice 6 persistence/events, Slice 7 manage API/OpenAPI foundation, Slice 8 report-input/AI-evidence handoff contracts, Slice 9 supportability/observability diagnostics, Slice 10 Gateway/Workbench realization RFC alignment, Slice 11 live manage proof, Slice 12 hardening, and Slice 13 closure are complete. Post-merge audit restored the cross-RFC work-to-be-done ledger, added RFC-0042 follow-up ownership, reran live manage proof at `output/rfc0042-outcome-proof/20260505-040212/`, and passed canonical front-office validation for stack-health only. Manage owns source-backed outcome reviews, while realized transaction, risk, and performance truth must come from owning apps. Gateway/Workbench product support remains downstream until implementation and canonical proof are complete. |
