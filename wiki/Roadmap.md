...
| RFC-0041 | CIO and PM teams can orchestrate book-level model-change and rebalance waves. | Implementation is complete through Slice 2: source-map, platform scaffold evidence improvement, and cleanup review. First-wave scope is explicit portfolio-list waves with mixed readiness proof; automatic PM-book and CIO model-change cohort discovery remain deferred until source-owning products are proven. Support remains proposed until manage backend implementation, live proof, downstream realization RFCs, and supported-feature promotion are complete. |
