...
| RFC-0041 | CIO and PM teams can orchestrate book-level model-change and rebalance waves. | Implementation is complete through Slice 5: source-map, platform scaffold evidence improvement, cleanup review, wave domain contracts, persistence foundation, explicit affected-portfolio preview, idempotent durable create, and durable source-check classification from mandate twin and mandate health evidence. First-wave scope remains explicit portfolio-list waves; automatic PM-book and CIO model-change cohort discovery remain deferred until source-owning products are proven. Support remains proposed until simulation, approval, handoff, live proof, downstream realization RFCs, and supported-feature promotion are complete. |
