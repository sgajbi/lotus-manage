...
| RFC-0041 | CIO and PM teams can orchestrate book-level model-change and rebalance waves. | Manage backend implementation is `DONE` for explicit portfolio-list waves and source-owned PM-book wave discovery: source-map, platform scaffold evidence improvement, cleanup review, wave domain contracts, Postgres-backed persistence, preview/create, `PM_BOOK_REVIEW` over lotus-core `PortfolioManagerBookMembership:v1`, mixed source-check, RFC-0039-backed ready-item simulation, item-level selection, RFC-0040 proof-pack linkage, approval-with-exceptions, staging, internal operations handoff evidence with no external execution claim, actor-attributed pre-execution cancellation, retrieve/search/item/proof-pack/supportability read models, product-safe diagnostics, OpenAPI certification, hardening review, final gold-pass assessment, and downstream Gateway/Workbench RFC-0098 wave realization addenda. CIO model-change cohort discovery remains deferred until source-owning products are proven. Full product support remains proposed until Gateway and Workbench implementation and canonical front-office proof are complete. |
