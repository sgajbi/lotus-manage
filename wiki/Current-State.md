...
| Mandate digital twin and command center | Mandate twin refresh/read/version/diff, health scoring, monitoring runs, exceptions, and command-center summary are implemented. Portfolio-mandate and health reads support optional source-preserving `as_of_date` selection without substituting future evidence. Mandate observations retain distinct business dates even when the source binding version is unchanged. | First-wave command-center path is implementation-backed through Gateway/Workbench where downstream proof exists. Historical downstream comparison remains evidence-alignment gated. |
...
| PM operating quality | Policy, score-run, fairness-analysis, review-action, and summary-invocation evidence lifecycles are implemented with trusted-identity tenant isolation, bounded PM-book membership, state-specific summary invocation evidence, support-only boundaries, and auditable scoring/fairness methodology at `docs/methodologies/pm-quality/scoring-and-fairness.md`. | Suitable for operating-quality governance demos. It is not a PM ranking, HR, compensation, conduct, trade approval, or autonomous decisioning engine. |
