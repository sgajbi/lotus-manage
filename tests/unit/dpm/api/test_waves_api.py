    ...
    *,
    ...
    return {
        ...
        "page": {
            ...
            "next_page_token": next_page_token if page_truncated else None,
        },
        "supportability": {
            "state": supportability_state,
            "reason": (
...
class _DpmPortfolioUniverseResolver:
    ...
    def resolve_dpm_portfolio_universe_candidates(self, **kwargs: object):
        ...
        return DpmCorePortfolioUniverseCandidateResponse.model_validate(
...
def test_wave_preview_returns_source_backed_and_blocked_items_without_persistence() -> None:
    mandate_repository = InMemoryDpmMandateRepository()
    mandate_repository.save_mandate_snapshot(_twin(), tenant_id="tenant-test")
    wave_repository = InMemoryDpmWaveRepository()
...
def test_bulk_review_campaign_preview_exhausts_core_portfolio_universe_candidate_pages(
    monkeypatch,
) -> None:
    ...
    with _client(mandate_repository, InMemoryDpmWaveRepository()) as client:
        response = client.post(
            "/api/v1/rebalance/waves/preview",
            json=_core_universe_bulk_review_campaign_request(),
            headers={"X-Correlation-Id": "corr-core-universe-paged-preview"},
        )

    assert response.status_code == 200
    payload = response.json()
    assert [item["portfolio_id"] for item in payload["wave"]["items"]] == [
        PORTFOLIO_ID,
        second_portfolio_id,
    ]
    assert [call["page_token"] for call in resolver.calls] == [None, "page-2"]
    assert all(
...
def test_bulk_review_campaign_definition_preview_readiness_is_ready_for_eligible_actor() -> None:
    ...
    assert payload["product_name"] == "BulkReviewCampaignDefinitionPreviewReadiness"
    assert payload["preview_create_allowed"] is True
...
def test_bulk_review_campaign_definition_preview_readiness_fails_closed() -> None:
    campaign_repository = InMemoryDpmBulkReviewCampaignDefinitionRepository()
    ...
    with _client(
        ...
        missing = client.get(
            ...
            "versions/2026.05/preview-readiness?requested_as_of_date=2026-05-10"
        )

    assert put_response.status_code == 200
    assert readiness.status_code == 200
    payload = readiness.json()
    assert payload["preview_create_allowed"] is False
...
def test_wave_source_check_classifies_mixed_items_and_attaches_authoritative_refs() -> None:
    ...
    mandate_repository.save_mandate_snapshot(ready_twin, tenant_id="tenant-test")
    mandate_repository.save_mandate_snapshot(degraded_twin, tenant_id="tenant-test")
...
def test_wave_simulation_reports_invalid_state_and_partial_result() -> None:
    ...
    mandate_repository.save_mandate_snapshot(ready_twin, tenant_id="tenant-test")
    mandate_repository.save_mandate_snapshot(second_twin, tenant_id="tenant-test")
    _save_ready_health(mandate_repository, ready_twin)
...
def test_wave_approval_excludes_blocked_items_and_stages_only_approved_items() -> None:
    ...
    mandate_repository.save_mandate_snapshot(ready_twin, tenant_id="tenant-test")
    mandate_repository.save_mandate_snapshot(blocked_twin, tenant_id="tenant-test")
    _save_ready_health(mandate_repository, ready_twin)
...
def test_wave_read_apis_return_durable_search_detail_items_and_proof_pack_posture() -> None:
    ...
    ] == list(range(1, len(report_payload["portfolio_memory_context"]["event_refs"]) + 1))
    assert any(
