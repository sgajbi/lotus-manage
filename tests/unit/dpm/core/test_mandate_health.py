...
def test_cashflow_pressure_does_not_hide_an_unavailable_cash_band() -> None:
    """Another condition in the same dimension must not mask the source gap.

    A negative projected cashflow returns PROJECTED_CASHFLOW_PRESSURE before
    the unsourced-band check runs, so CASH_BAND_NOT_SOURCED never becomes this
    dimension's reason. The headline action would then say SIMULATE_REBALANCE
    while the contractual limit is still unavailable - sending an operator to
    simulate against a band that does not exist.

    The absence is a fact about the twin, so the action reads it from the
    twin's field_gap_codes rather than depending on which condition happened
    to win the dimension.
    """
    ...
    # But the headline does not tell an operator to simulate against a band
