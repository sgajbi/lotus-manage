...
def test_rfc0041_slice0_source_map_guardrails_stay_truthful() -> None:
    ...
    assert "IN PROGRESS - SLICE 6 SIMULATION/SELECTION COMPLETE" in rfc
    assert "IN PROGRESS - SLICE 6 SIMULATION/SELECTION COMPLETE" in index
    ...
    assert "implementation is in progress through Slice 6" in wiki_index_normalized
    assert "Implementation is complete through Slice 6" in roadmap
