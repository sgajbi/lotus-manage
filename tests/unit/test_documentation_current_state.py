...
def test_rfc0036_completed_wtbd_truth_is_integrated_into_rfc_and_wiki() -> None:
    ...
    assert "Completed: promote the current additional stateful DPM source-readiness product" in rfc
...
def test_rfc0040_completed_wtbd_truth_is_integrated_into_rfc_and_wiki() -> None:
    ...
    required_rfc_terms = [
        ...
        "direct source-owned scenario-pack enrichment",
        "RFC40-WTBD-009",
    ...
    required_wiki_terms = [
        ...
        "direct source-owned `regime_stress_context` enrichment",
...
def test_rfc0042_gold_standard_tightening_preserves_source_boundaries() -> None:
    ...
    assert (
        "PM operating quality policy, score-run, fairness-analysis, review-action, and summary-invocation lifecycle | Supported as "
    ...
    assert "PmOperatingQualitySummaryInvocation:v1" in rfc
    ...
    assert "/api/v1/rebalance/pm-operating-quality/summary-invocations/preview" in rfc
    ...
    assert "rolling volatility methodology truth" in supported_features
    assert "`lotus-risk` PR #117" in supported_features
    assert "wiki `c6eef3c`" in supported_features
    assert "rolling Sharpe methodology truth" in supported_features
    assert "`lotus-risk` PR #118" in supported_features
    assert "wiki `0b96201`" in supported_features
    ...
    assert "top-position weight methodology truth through `lotus-risk` PR #134" in (
        supported_features
    )
    ...
    assert "risk drawdown methodology truth" in supported_features
    assert "`lotus-risk` PR #128" in supported_features
    assert "wiki `edde5df`" in supported_features
    ...
    assert "risk tracking-error methodology truth" in supported_features
    assert "`lotus-risk` PR #124" in supported_features
    assert "wiki `a1d8898`" in supported_features
    assert "risk information-ratio methodology truth" in supported_features
    assert "`lotus-risk` PR #125" in supported_features
    assert "wiki `7a0aa9e`" in supported_features
    ...
    assert "`POST /api/v1/rebalance/pm-operating-quality/summary-invocations`" in work_to_be_done
    assert (
    ...
    assert "PR #314 (`6bd43c8`)" in work_to_be_done
    assert "review-action preview-before-create command UX" in work_to_be_done
    ...
    assert "Snapshot basis: the 2026-05-24 clean mainline" in work_to_be_done
    ...
    assert "| Partial / in progress | 0 |" in work_to_be_done
    assert "| Remaining / open | 1 |" in work_to_be_done
    assert "| Done on merged/published Lotus-owned truth | 57 |" not in work_to_be_done
    assert "| Partial / in progress | 1 |" not in work_to_be_done
    ...
    assert (
        "`GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/"
        "{campaign_version}/launch-package`" in work_to_be_done
    )
    assert (
        "`GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/"
    ...
    assert (
        "`POST /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/"
        "{campaign_version}/assignment-tasks`" in work_to_be_done
    )
    assert (
        "`POST /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/"
        "{campaign_version}/assignment-tasks/{task_ref}/transitions`" in work_to_be_done
    )
    ...
    assert "Latest WTBD-006 risk rolling-volatility methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #117" in work_to_be_done
    assert "`8f04b24276bd73dd34ef5ce3edf59e81453858ae`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `c6eef3c`" in work_to_be_done
    assert "`ROLLING_VOLATILITY` source-owner methodology" in work_to_be_done
    assert "constant-return zero-volatility behavior" in work_to_be_done
    assert "source-owner proof passed locally with `318` unit tests" in work_to_be_done
    assert "Latest WTBD-006 risk rolling-Sharpe methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #118" in work_to_be_done
    assert "`3f4bbfe6d536a1f3d68b773f917a3f059987db51`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `0b96201`" in work_to_be_done
    assert "`ROLLING_SHARPE` source-owner methodology" in work_to_be_done
    assert "zero-excess-volatility flagging" in work_to_be_done
    assert "source-owner proof passed locally with `320` unit tests" in work_to_be_done
    assert "Latest WTBD-006 risk rolling-beta methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #119" in work_to_be_done
    assert "`ffcfddbc24484811152c85582398528c0c879d98`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `bcccb0c`" in work_to_be_done
    ...
    assert "Latest WTBD-006 risk concentration top-position methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #134" in work_to_be_done
    assert "`21ef697a1c308a3d8ea7c8e40e06019544be7e93`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `dd25844`" in work_to_be_done
    assert "`TOP_POSITION_WEIGHT`" in work_to_be_done
    assert "decimal `0..1` top-position weight output" in work_to_be_done
    assert "deterministic top-position driver selection" in work_to_be_done
    ...
    assert "Latest WTBD-006 risk Sharpe methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #122" in work_to_be_done
    assert "`932600162df7482a6d9c01a7470760238cab57ce`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `cdb25df`" in work_to_be_done
    assert "`SHARPE` source-owner methodology" in work_to_be_done
    ...
    assert "Latest WTBD-006 risk Sortino methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #126" in work_to_be_done
    assert "`dbe16b647f972ff626a8b6eb11e06041b6e0f46a`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `81f787e`" in work_to_be_done
    ...
    assert "Latest WTBD-006 risk drawdown methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #128" in work_to_be_done
    assert "`4784839067027d991500ff2d19c728122e227466`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `edde5df`" in work_to_be_done
    assert "`DRAWDOWN` source-owner methodology" in work_to_be_done
    assert "`metrics.DRAWDOWN.value`" in work_to_be_done
    assert "cumulative-wealth and running-peak behavior" in work_to_be_done
    assert "source-owner proof passed locally with `340` unit tests" in work_to_be_done
    ...
    assert "Latest WTBD-006 risk tracking-error methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #124" in work_to_be_done
    assert "`46eddb4dc332bb1e4a79c22b1bd557f2b2db2cb9`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `a1d8898`" in work_to_be_done
    assert "`TRACKING_ERROR` source-owner methodology" in work_to_be_done
    assert "annualized percentage-point `metrics.TRACKING_ERROR.value`" in work_to_be_done
    assert "source-owner proof passed locally with `332` unit tests" in work_to_be_done
    assert "Latest WTBD-006 risk information-ratio methodology proof" in work_to_be_done
    assert "`lotus-risk` PR #125" in work_to_be_done
    assert "`7ccf5667e6676b8f1865ebd9768b8913262d3d3b`" in work_to_be_done
    assert "`lotus-risk.wiki` commit `7a0aa9e`" in work_to_be_done
    assert "`INFORMATION_RATIO` source-owner methodology" in work_to_be_done
    assert "dimensionless annualized `metrics.INFORMATION_RATIO.value`" in work_to_be_done
    assert "source-owner proof passed locally with `334` unit tests" in work_to_be_done
    ...
    assert "`ae4f707`" in work_to_be_done
    assert "`9e86df3b`" in work_to_be_done
    assert "`617e4e6`" in work_to_be_done
    assert "`763db4c1`" in work_to_be_done
    assert "`50fff30`" in work_to_be_done
    ...
    assert "`RiskMetricsReport:v1`, `DrawdownAnalyticsReport:v1`" in supported_features
    assert "`BenchmarkExposureContext:v1`, `ContributionAnalytics:v1`" in supported_features
    ...
    assert (
        "`POST` and\n"
        "`GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/"
        "{campaign_version}/assignment-tasks`" in supported_features
    )
    ...
    assert "append-only campaign approval decisions" in supported_features
