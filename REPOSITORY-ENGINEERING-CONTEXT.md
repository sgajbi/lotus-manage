...
6. it carries the RFC-0091 repo-native producer declaration and telemetry fixture for
   ...
   `available`. Manage now preserves source-owned Risk/Performance mandate-health `as_of_date`
   metadata when the producer supplied it and degrades to `missing_source_evidence` or
   `mixed_source_as_of` for older, incomplete, or inconsistent evidence. Trusted tenant/session
...
13. RFC-0039 has delivered the implementation-backed construction-alternative backend foundation:
    ...
    active fail-closed `UNAVAILABLE` route, and `lotus-core` PR #369 (`89225766`, wiki `72dc91d`)
    exposes `ExternalFXForwardCurve:v1` as an active fail-closed `UNAVAILABLE` route, and
    `lotus-core` PR #370 (`bacad356`, wiki `6e7c706`) exposes
    `ExternalEligibleHedgeInstrument:v1` as an active fail-closed `UNAVAILABLE` route.
    ...
    attribution, hedge-policy approval, eligible-instrument selection, suitability approval,
    product recommendation, hedge advice, forward pricing, FX valuation methodology, counterparty
    selection, treasury instruction, best-execution, OMS, fill, and settlement claims.
...
14. RFC-0040 has delivered the implementation-backed manage pre-trade proof-pack backend
    ...
    closed when any score-contributing source evidence lacks a valid ISO `YYYY-MM-DD` business date
    or falls outside the approved lookback window without discovering peers, ranking PMs, or owning
    source methodology. Under a lookback policy, the first `source_ref.source_version` on each
    signal is the governing business date; the policy timezone names the business calendar and does
    not allow timestamp conversion or month-version fallback.
...
15. RFC-0041 is `DONE` for implementation-backed manage backend authority over explicit
    ...
    and blocks new wave preview/create use. Manage can also supersede an active definition with an
