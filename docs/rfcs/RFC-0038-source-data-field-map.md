...
| `constraints.cash_band_min_weight` | Not yet available | Null. A mandate cash band is a contractual limit and no source product states one; the absence is reported as `MANDATE_CASH_BAND_NOT_YET_SOURCED` and keeps `CASH_LIQUIDITY` out of a READY assessment. | Needs an explicit mandate cash-band source. |
| `constraints.cash_band_max_weight` | Not yet available | Null, with the same gap code and the same effect on health. Manage does not derive a ceiling from the cash reserve: a rebalance reserve is not a mandate band. | Needs an explicit mandate cash-band source. |
| `constraints.turnover_budget` | Not yet available | Null, reported as `MANDATE_TURNOVER_BUDGET_NOT_YET_SOURCED` and keeping `TAX_TURNOVER` out of a READY assessment. | Policy-pack or mandate restriction source should own this. |
| `constraints.cash_reserve_weight` | `DiscretionaryMandateBinding:v1.rebalance_bands.cash_reserve_weight` | Source-backed, under its own meaning: an operating floor for rebalancing, never reinterpreted as a mandate band boundary. | None. |
...
| `CASH_LIQUIDITY` | Cash weight, mandate cash band, and optional `PortfolioCashflowProjection:v1.total_net_cashflow` | Pending review when below/above band. Negative source-owned projected net cashflow creates `PROJECTED_CASHFLOW_PRESSURE` when current cash is within band. **No source supplies a mandate cash band today, so in practice every Core-compiled twin reaches `CASH_BAND_NOT_SOURCED` at pending review with a `FIX_SOURCE_DATA` action - an unassessable limit is never a ready result.** | Client income-need profile, settlement ladder, overdraft risk, and richer horizon semantics. |
| `TAX_TURNOVER` | Missing tax-lot securities and turnover budget usage | Missing tax lots block; turnover usage above 80% of budget creates pending review. **No source supplies a turnover budget today, so absent a budget the dimension reaches `TURNOVER_BUDGET_NOT_SOURCED` at pending review with a `FIX_SOURCE_DATA` action rather than a ready result.** | Tax budget source, realized gain forecast, wash-sale/local tax rules where applicable. |
