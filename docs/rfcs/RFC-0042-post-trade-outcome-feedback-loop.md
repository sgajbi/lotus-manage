...
2. PM operating quality now has a bounded Manage-owned policy administration, score-run preview,
   ...
   projection, and source-defined segment fairness spread posture without protected-class inference
   or PM ranking; persisted fairness analyses are content-addressed and returned without
   recomputation; downstream review-action BFF/UI realization remains future expansion,
...
UI, and richer source-owner methodology remain explicit future scope, not hidden gaps in the
supported path. PM operating quality first-wave policy administration, preview, score-run
lifecycle, fairness-analysis lifecycle, optional source-owned PM-book scope materialization, and
bounded portfolio-memory score-run lineage projection are now owned by `lotus-manage` and
