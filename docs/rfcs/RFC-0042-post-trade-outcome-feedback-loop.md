...
2. PM operating quality now has a bounded Manage-owned policy administration, score-run preview,
   score-run lifecycle, and fairness-analysis preview product with explicit
   ...
   posture without protected-class inference or PM ranking; downstream UI remains future expansion,
...
lifecycle, fairness-analysis preview, optional source-owned PM-book scope materialization, and
...
| RFC42-WTBD-008 | `lotus-manage` implements the first bounded PM operating quality policy administration, score-run preview, lifecycle product, source-segment fairness-analysis preview, and portfolio-memory score-run lineage projection at `PUT /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `GET /api/v1/rebalance/pm-operating-quality/policies`, `GET /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `POST /api/v1/rebalance/pm-operating-quality/score-runs/preview`, `POST /api/v1/rebalance/pm-operating-quality/score-runs`, `GET /api/v1/rebalance/pm-operating-quality/score-runs`, `GET /api/v1/rebalance/pm-operating-quality/score-runs/{score_run_id}`, `POST /api/v1/rebalance/pm-operating-quality/fairness-analyses/preview`, and `GET /api/v1/rebalance/portfolio-memory/{portfolio_id}`, emitting and persisting `PmOperatingQualityScoreRun:v1` from explicit bank policy, governance approval, fairness-review evidence, source-owned evidence, optional persisted outcome reviews, optional lotus-core `PortfolioManagerBookMembership:v1` PM-book scope evidence, and optional bank-defined peer-group/lookback scope evidence. `lotus-gateway` PR #213 (`62ce4c4`) exposes the corresponding `/api/v1/dpm/command-center/pm-operating-quality/*` BFF route family and published Gateway wiki source at `a4c9db9`. | Scoring is disabled by default; enabled policies fail closed for missing evidence, missing approval, invalid/expired expiry, unauthorized actors, and dated evidence outside approved lookback windows, and reject prohibited use. Gateway preserves Manage truth without calculating scores, ranking PMs, administering policy locally, approving trades, contacting clients, routing orders, claiming execution, or creating HR, compensation, conduct, or autonomous-ranking decisions. PM-book materialization fails closed for unavailable, incomplete, degraded, or empty source membership. Peer-group/lookback materialization records source refs and `scope_evidence` only; Manage does not discover peers or own source methodology. Fairness-analysis preview consumes persisted score-run ids and source-defined segment refs, validates common policy/as-of scope, minimum scorable segment counts, and governed average-score spread, and does not infer protected classes or create PM rankings. Portfolio memory projects only bounded `PM_QUALITY_SCORE_RUN` lineage for matching source-backed PM-book members and omits raw score payloads. Workbench PM-quality UI and Gateway/Workbench fairness-analysis product realization remain future expansion. |
