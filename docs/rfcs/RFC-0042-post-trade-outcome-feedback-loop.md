...
portfolio-memory score-run, review-action, and summary-invocation lineage projection are also
implemented for that product. Manage now
...
4. `EXECUTION_EVIDENCE_BLOCKED` is emitted where execution truth is unavailable, and
   supportability plus report/AI handoffs emit `DPM_OUTCOME_EXTERNAL_EXECUTION_BOUNDARY` with
   blocked capabilities, required future execution/OMS owner, required
   `ExternalOrderExecutionAcknowledgement:v1`, acknowledgement-count posture, and deterministic
   content hash.
...
administration, score-run lifecycle, fairness-analysis lifecycle, and review-action lifecycle are
supported as a separate Manage-owned product; score-run PM-book materialization through lotus-core
`PortfolioManagerBookMembership:v1` is supported when explicitly requested, and bounded
portfolio-memory score-run, review-action, and summary-invocation lineage is supported for
persisted source-backed PM-book members. Gateway BFF composition under
`/api/v1/dpm/command-center/pm-operating-quality/*` is implementation-backed for policy,
score-run, fairness-analysis, and score-run summary handoff APIs through `lotus-gateway` PR #213
(`62ce4c4`) and published Gateway wiki source at `a4c9db9`, preserving Manage truth without local
scoring, ranking, policy administration, HR, compensation, conduct, approval, client-contact, or
execution decisions. Gateway review-action BFF routes and Workbench review-action
ledger/detail/preview/create UX are implementation-backed in their owning repositories. Manage
review actions now carry structured `PM_QUALITY_APPROVAL_WORKFLOW_BOUNDARY` evidence so consumers
can distinguish immutable review-action ledger evidence from unsupported CIO approval workflow,
policy approval, client approval, trade approval, HR/conduct, order-routing, or OMS execution
claims. Execution/OMS integration, client communication, persisted PM-quality summary history,
approval workflow beyond immutable review actions, and richer source-owner methodology remain
...
| Gateway outcome composition | Supported through `lotus-gateway` PR #186/#187/#188/#189, with source-lineage search/facet realization through PR #242 | Gateway composes manage outcome-review truth and report/AI handoff posture without recomputing expected values, realized values, variance, tolerance, lineage, freshness, or review state. PR #242 forwards bounded Manage-owned source-lineage filters and preserves applied-filter, facet, and support-boundary posture without querying source-owner stores. |
| Workbench outcome review UX | Supported through `lotus-workbench` PR #146/#147/#148, with source-lineage filter/facet rendering through PR #350 | Workbench consumes Gateway/BFF contracts only and renders the first-wave outcome-review, report-request, AI-narrative request, and source-lineage filter/facet posture with canonical proof. PR #350 remains Gateway-only and does not synthesize realized source truth, query source-owner stores, project OMS events, or create client-communication workflow evidence. |
| PM operating quality policy, score-run, fairness-analysis, review-action, and summary-invocation lifecycle | Supported as separate Manage-owned first-wave product | `PUT /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `GET /policies`, and `GET /policies/{policy_id}/versions/{policy_version}` administer immutable policy versions; `POST /score-runs/preview` emits `PmOperatingQualityScoreRun:v1`; `POST /score-runs`, `GET /score-runs`, and `GET /score-runs/{score_run_id}` persist and retrieve immutable score-run evidence; `POST /fairness-analyses/preview` emits bounded `PmOperatingQualityFairnessAnalysis:v1` over persisted score runs and source-defined segments; `POST /fairness-analyses`, `GET /fairness-analyses`, and `GET /fairness-analyses/{fairness_analysis_id}` persist, list, and retrieve immutable fairness-analysis evidence without recomputing score runs; `POST /review-actions/preview`, `POST /review-actions`, `GET /review-actions`, and `GET /review-actions/{review_action_id}` emit immutable `PmOperatingQualityReviewAction:v1` ledger rows over existing score-run or fairness-analysis evidence without recalculating scores or recomputing fairness, and each row carries structured `PM_QUALITY_APPROVAL_WORKFLOW_BOUNDARY` evidence with a deterministic content hash; `POST /summary-invocations/preview`, `POST /summary-invocations`, `GET /summary-invocations`, and `GET /summary-invocations/{summary_invocation_id}` emit immutable `PmOperatingQualitySummaryInvocation:v1` history rows over existing score-run and review-action evidence without storing generated AI narrative text, exposing raw review rationale, recalculating scores, recomputing fairness, ranking PMs, or creating client-contact, trade, order, OMS, or execution claims. Enabled policies require bank approval and fairness-review evidence; score-run preview/create emits `governance_evidence` and fails closed for missing approval, invalid/expired expiry, unauthorized actors, and missing scoring evidence. Optional `pm_book_scope` materializes source-owned lotus-core `PortfolioManagerBookMembership:v1` scope evidence and fails closed for unavailable, incomplete, degraded, or empty PM-book membership. Optional `peer_group_policy` and `lookback_window_policy` materialize into score-run `scope_evidence`, preserve peer-group/lookback source refs in the content hash, and fail closed with `PM_QUALITY_LOOKBACK_WINDOW_EVIDENCE_DATE_REQUIRED`, `PM_QUALITY_EVIDENCE_AS_OF_DATE_INVALID`, or `PM_QUALITY_EVIDENCE_OUTSIDE_LOOKBACK_WINDOW` when score-contributing evidence lacks an ISO `YYYY-MM-DD` business date, carries an invalid date, or falls outside the approved window. The first `source_ref.source_version` on each signal is the governing business date when lookback is enabled; policy timezone is business-calendar metadata, not timestamp conversion or month-version fallback. Fairness analysis validates common policy/as-of scope, requires minimum scorable segment counts, flags governed average-score spread review, persists content-addressed evidence immutably, and does not infer protected classes or rank PMs. Scoring is disabled by default, and HR, compensation, conduct-enforcement, autonomous-ranking, AI-generated scoring, local peer discovery, approval-workflow, and source-owner methodology claims are prohibited. |
...
| PM scoring is inferred from outcome facts | Keep outcome reviews factual. PM operating quality is only available through the separate explicit policy administration and score-run preview/create/read/list routes with bank policy, source evidence, non-use posture, immutable storage, and bounded portfolio-memory score-run, review-action, and summary-invocation lineage that omits raw score payloads, raw review rationale, generated summary text, prompt bodies, model responses, downstream summary UX, and portfolio-level rankings. |
...
2. PM operating quality now has a bounded Manage-owned policy administration, score-run preview,
   score-run lifecycle, fairness-analysis preview/create/read/list product, and immutable
   review-action preview/create/read/list ledger with explicit
   ...
   PM-book scope materialization, bank-defined `peer_group_policy` and `lookback_window_policy`
   materialized as score-run `scope_evidence` with fail-closed dated-evidence validation, bounded
   portfolio-memory score-run lineage projection, portfolio-memory review-action supervisory
   projection, bounded portfolio-memory summary-invocation workflow-lineage projection, Gateway
   review-action BFF realization, and Workbench review-action ledger/detail
   plus preview-before-create command UX without protected-class inference or PM ranking; persisted
   fairness analyses are content-addressed and returned without recomputation,
...
as the evidence authority. Execution/OMS integration, client communication, approval workflow
beyond immutable review actions, and richer
source-owner methodology remain explicit future scope, not hidden gaps in the supported path. PM
operating quality first-wave policy administration, preview, score-run lifecycle, fairness-analysis
lifecycle, immutable review-action ledger/detail and preview-before-create command UX, persisted
summary-invocation history list/detail/command downstream realization, optional source-owned
PM-book scope materialization, and bounded portfolio-memory score-run, review-action, and
summary-invocation lineage projection are now implementation-backed with strict policy,
governance, evidence, storage, and non-use boundaries.
...
| RFC42-WTBD-006 | Source-owner methodology enrichment has progressed across risk, performance, and core source products and is consumed by manage adapters where implemented. `lotus-performance` now has merged source-owner methodology and evidence truth for RFC-046 TWR daily calculation evidence, supportability, benchmark FX/calendar posture, stateful MWR, contribution, and attribution source-resolution boundaries through PR #144 (`37e125b6`), PR #145 (`7aa83fe5`), PR #146 (`817c5bbc`), and PR #156 (`bf173b4`). Live audit on 2026-05-10 confirmed current canonical `PB_SG_GLOBAL_BAL_001` performance evidence through `POST /performance/twr`, Gateway performance summary/details, canonical TWR inspection, Workbench live validation, and clean performance/gateway logs. `TransactionLedgerWindow:v1` now restates reporting-currency fields by source-currency basis: book-currency measures use book currency, trade/local measures use trade currency when present, and explicit row-level realized FX P&L local evidence is exposed through `realized_fx_pnl_local_reporting_currency` for reporting surfaces. Canonical front-office seed data also carries USD/SGD and EUR/SGD reporting-currency FX coverage for live proof from `lotus-core` PR #359. `PortfolioCashflowProjection:v1` now emits total, booked, and projected-settlement cashflow measures, and `PortfolioLiquidityLadder:v1` now emits opening cash, fixed horizon bucket, booked/projected/net cashflow, cumulative cash, shortfall, and asset-liquidity-tier exposure evidence from `lotus-core` PR #356 / wiki `28c4ae2`. `lotus-core` PR #361 (`ec75ddfc`) implements source-owned `ClientTaxProfile:v1` and `ClientTaxRuleSet:v1` contracts, ingestion, query/control-plane APIs, supportability, methodology docs, route-family governance, source-product catalog/security profiles, and wiki publication (`2f47f65`) for bounded tax profile/rule evidence potentially mapped from external bank/tax systems. `lotus-core` PR #362 (`c1fb350c`) implements source-owned `ClientIncomeNeedsSchedule:v1`, `LiquidityReserveRequirement:v1`, and optional `PlannedWithdrawalSchedule:v1` contracts, ingestion, query/control-plane APIs, supportability, methodology docs, route-family governance, source-product catalog/security profiles, and wiki publication (`40a1228`) for bounded client-liquidity reference evidence. `lotus-core` PR #363 (`a349dc0`) implements source-owned `PortfolioRealizedTaxSummary:v1` for portfolio-level explicit realized-tax evidence, withholding-tax and other-interest-deduction aggregation by ledger currency, optional reporting-currency restatement, methodology docs, route-family governance, source-product catalog/security profiles, and wiki publication (`1170afd`); `lotus-platform` PR #331 (`500a1b8`) mirrors the product into platform mesh discovery and certification artifacts. `lotus-core` PR #364 (`486136a`, wiki `ad67cf6`) implements source-owned `PortfolioCashMovementSummary:v1` signed cash-movement bucket evidence. `lotus-manage` now preserves both `PortfolioRealizedTaxSummary:v1` portfolio totals and `PortfolioCashMovementSummary:v1` bucket totals as RFC42 realized source snapshots without local tax aggregation, cashflow aggregation, forecasting, advice, treasury instruction, execution-quality assessment, or OMS acknowledgement. Gateway PR #242 and Workbench PR #350 realize the downstream bounded source-lineage filters/facets over Manage-persisted review lineage; they do not change source-owner methodology completion. | After-tax optimization, tax advice, tax-loss harvesting suitability, jurisdiction-specific recommendations, client-tax approval, tax-reporting certification, portfolio-level FX attribution beyond source-owned performance attribution, income-needs planning, financial-planning advice, funding recommendations, predictive execution, and OMS acknowledgements remain source-owner follow-on work or unsupported claims. |
| RFC42-WTBD-007 | Lotus-side fail-closed external execution/OMS boundary truth is partial: `lotus-core` exposes `ExternalOrderExecutionAcknowledgement:v1` as `UNAVAILABLE`, and `lotus-manage` preserves that posture in construction diagnostics, realized outcome `EXECUTION_QUALITY` evidence, supportability, report input, and AI evidence input through `DPM_OUTCOME_EXTERNAL_EXECUTION_BOUNDARY`. | Production OMS integration remains unsupported until a bank-owned execution/OMS owner provides certified ingestion controls, acknowledgements, fills, settlement, and reconciliation contracts. |
| RFC42-WTBD-008 | `lotus-manage` implements the first bounded PM operating quality policy administration, score-run preview, lifecycle product, source-segment fairness-analysis preview/create/read/list lifecycle, immutable review-action preview/create/read/list ledger, immutable support-summary invocation history preview/create/read/list lifecycle, portfolio-memory score-run lineage projection, portfolio-memory review-action supervisory projection, and portfolio-memory summary-invocation workflow-lineage projection at `PUT /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `GET /api/v1/rebalance/pm-operating-quality/policies`, `GET /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `POST /api/v1/rebalance/pm-operating-quality/score-runs/preview`, `POST /api/v1/rebalance/pm-operating-quality/score-runs`, `GET /api/v1/rebalance/pm-operating-quality/score-runs`, `GET /api/v1/rebalance/pm-operating-quality/score-runs/{score_run_id}`, `POST /api/v1/rebalance/pm-operating-quality/fairness-analyses/preview`, `POST /api/v1/rebalance/pm-operating-quality/fairness-analyses`, `GET /api/v1/rebalance/pm-operating-quality/fairness-analyses`, `GET /api/v1/rebalance/pm-operating-quality/fairness-analyses/{fairness_analysis_id}`, `POST /api/v1/rebalance/pm-operating-quality/review-actions/preview`, `POST /api/v1/rebalance/pm-operating-quality/review-actions`, `GET /api/v1/rebalance/pm-operating-quality/review-actions`, `GET /api/v1/rebalance/pm-operating-quality/review-actions/{review_action_id}`, `POST /api/v1/rebalance/pm-operating-quality/summary-invocations/preview`, `POST /api/v1/rebalance/pm-operating-quality/summary-invocations`, `GET /api/v1/rebalance/pm-operating-quality/summary-invocations`, `GET /api/v1/rebalance/pm-operating-quality/summary-invocations/{summary_invocation_id}`, and `GET /api/v1/rebalance/portfolio-memory/{portfolio_id}`, emitting and persisting `PmOperatingQualityScoreRun:v1` from explicit bank policy, governance approval, fairness-review evidence, source-owned evidence, optional persisted outcome reviews, optional lotus-core `PortfolioManagerBookMembership:v1` PM-book scope evidence, and optional bank-defined peer-group/lookback scope evidence. Persisted `PmOperatingQualityFairnessAnalysis:v1` evidence is immutable, content-addressed, queryable by policy/as-of/state, and returned without recomputing score runs. Persisted `PmOperatingQualityReviewAction:v1` evidence is immutable, content-addressed, queryable by target/policy/as-of/state, preserves target content hashes, and records bounded bank review rationale without recalculating scores or recomputing fairness. Persisted `PmOperatingQualitySummaryInvocation:v1` evidence is immutable, content-addressed, queryable by score-run/review-action/policy/as-of/state, and records state-specific workflow, completed artifact, hash, or bounded failure evidence without storing generated AI narrative text or raw review rationale. `lotus-gateway` PR #213 (`62ce4c4`) exposes policy, score-run, fairness-analysis, and score-run AI-summary handoff BFF routes and published Gateway wiki source at `a4c9db9`; Gateway PR #234 (`c74b3e1`) exposes review-action preview/create/list/detail BFF routes over Manage truth. `lotus-workbench` PR #302 (`8052d92`) and PR #303 (`3581b04`) add read-only review-action ledger/detail support, PR #314 (`6bd43c8`) adds Gateway-only review-action preview-before-create command UX, PR #315 (`806193f`) extracts the bounded command-control component, and PR #316 (`fdc37b5`) extracts the PM-quality API module; Workbench Main Releasability run `26206758251` is green at `1dfdeb14`. `lotus-ai` PR #70 (`1951f62`, wiki `038a1a1`) adds `pm_quality_summary.pack@v1` for review-gated support-only summaries over Manage-owned `PmOperatingQualityScoreRun` evidence. | Scoring is disabled by default; enabled policies fail closed for missing evidence, missing approval, invalid/expired expiry, unauthorized actors, and dated evidence outside approved lookback windows, and reject prohibited use. Gateway and Workbench preserve Manage truth without calculating scores, ranking PMs, administering policy locally, approving trades, contacting clients, routing orders, claiming execution, or creating HR, compensation, conduct, or autonomous-ranking decisions. The AI summary pack validates score-run identity, source refs, supportability posture, optional bounded portfolio-memory context, and forbidden-use controls without calculating scores, ranking PMs, generating HR/compensation/conduct decisions, contacting clients, approving trades, routing orders, claiming execution, or inventing missing facts. PM-book materialization fails closed for unavailable, incomplete, degraded, or empty source membership. Peer-group/lookback materialization records source refs and `scope_evidence` only; Manage does not discover peers or own source methodology. Fairness-analysis preview/create consumes persisted score-run ids and source-defined segment refs, validates common policy/as-of scope, minimum scorable segment counts, and governed average-score spread, and does not infer protected classes or create PM rankings. Review actions consume existing score-run or fairness-analysis ids, preserve reviewed evidence hashes, and do not recalculate scores, recompute fairness, approve trades, contact clients, route orders, claim execution, or create HR, compensation, conduct, or autonomous-ranking decisions. Summary invocations consume existing score-run and review-action ids, preserve evidence hashes, and do not store generated AI text, expose raw rationale, recalculate scores, recompute fairness, approve trades, contact clients, route orders, claim execution, or create HR, compensation, conduct, or autonomous-ranking decisions. Portfolio memory projects bounded `PM_QUALITY_SCORE_RUN` lineage, bounded `PM_QUALITY_REVIEW_ACTION` supervisory events, and bounded `PM_QUALITY_SUMMARY_INVOCATION` workflow-lineage events for matching source-backed PM-book members while omitting raw score payloads, raw review rationale, generated summary text, prompt bodies, model responses, and downstream summary UX. Generated-summary text retention, approval workflow beyond immutable review actions, and external OMS/execution remain future expansion. |

2026-05-22 downstream realization backfill: Gateway PR #239 (`c4f007d`) adds bounded PM-quality
summary-invocation preview/create/list/detail BFF routes over Manage truth. Workbench PR #341
(`667d18c`) renders persisted summary-invocation list/detail evidence, PR #342 (`565a555`) adds the
...
than a new product capability. This strengthens downstream realization for the already-completed
bounded RFC42-WTBD-008 support claim; it does not change WTBD counts and does not promote generated
summary text retention, prompt/model-response exposure, PM ranking, HR/conduct action,
client-contact, trade, order, OMS, execution, fill, or settlement claims.
...
9. The 2026-05-21 PM-quality summary-text boundary hardening adds structured
   ...
   records state-specific workflow, completed artifact, hash, or bounded failure evidence only; it does not store or expose generated
