...
for persisted source-backed PM-book members. Gateway BFF composition for policy and score-run APIs
is implementation-backed through `lotus-gateway` PR #213 (`62ce4c4`) and published Gateway wiki
source at `a4c9db9`, preserving Manage truth without local scoring, ranking, policy
administration, HR, compensation, conduct, approval, client-contact, or execution decisions.
Execution/OMS integration, client communication, Workbench PM quality UI, advanced cross-segment
fairness analytics, and richer source-owner methodology remain outside the support claim.
...
2. PM operating quality now has a bounded Manage-owned policy administration, score-run preview,
   score-run lifecycle, fairness-analysis preview/create/read/list product, and immutable review-action preview/create/read/list ledger with explicit
   ...
   portfolio-memory score-run lineage projection, and source-defined segment fairness spread
   posture without protected-class inference or PM ranking; persisted fairness analyses are
   content-addressed and returned without recomputation; downstream UI remains future expansion,
