...
| RFC40-WTBD-010 | Manage, Gateway, Workbench, Report, AI, and Archive now preserve first-wave portfolio-memory lineage across proof-pack, wave, outcome, report, AI, generated-document archive, supersession, correction, and client-delivery reissue flows. `lotus-archive` PR #25 exposes `GET /documents/{document_id}/source-events` for the `lotus-archive.generated_document_client_communication.v1` family, merged as `aa3a3a8f28b666cb85100c0859f77ff2dab9cede` and wiki-published as `d5e5918`. Manage now also publishes `source_event_family_posture` so supported manage/report/AI/archive families are named in the API contract, external OMS execution is visible as a `DEFERRED_SOURCE_OWNER` boundary, and PM scoring points to the separate Manage-owned PM operating quality score-run lifecycle product without projecting hidden portfolio-memory score events. | The current portfolio-memory support claim is complete without hidden execution or scoring support. Future OMS execution, PM quality policy administration, PM-book materialization, UI, and score-event projection remain separate roadmap scope. Archive source events deliberately exclude raw document bytes, storage keys, raw report payloads, and raw client references. |
