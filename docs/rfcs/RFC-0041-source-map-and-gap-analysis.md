...
| First-wave trigger subset defined | First implementation scope began with explicit portfolio-list waves. Source-owned `PM_BOOK_REVIEW` and `CIO_MODEL_CHANGE` cohort discovery are now implementation-backed through lotus-core source products. Tactical house-view, risk-event, and bulk-campaign cohorts remain deferred until owning source products exist. |
