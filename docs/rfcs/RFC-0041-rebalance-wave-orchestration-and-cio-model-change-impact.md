...
| **Downstream Realization Depends On** | First-wave Gateway/Workbench wave command center, report/render/archive materialization, and AI PM memo handoff are implementation-backed; bounded manage risk-event consumption now uses `lotus-risk` `RiskEventAffectedCohort:v1`; bounded tactical house-view consumption now uses `lotus-advise` `TacticalHouseViewAffectedCohort:v1`; campaign/bulk-review membership is implemented as manage-owned `BulkReviewCampaignMembership:v1` over source-backed candidate portfolios with optional approval, expiry, actor-entitlement governance evidence, persisted `BulkReviewCampaignDefinition:v1` definitions, persisted `BulkReviewCampaignDiscovery:v1`, bounded campaign operating queue, bounded read-only approval-attention inbox, bounded read-only campaign workflow board, bounded read-only campaign assignment plan, campaign-definition retirement for definitions closed to new wave use, campaign-definition supersession for older versions replaced by active versions, campaign-definition lifecycle-event projection for bounded audit including launch history, campaign-definition preview-readiness checks for fail-closed new preview/create supportability, campaign-definition launch packages for bounded preview/create request and idempotency guidance, and durable campaign-definition launch when readiness is `READY`; external OMS execution, global portfolio-universe discovery, maker-checker workflow, and mutable assignment/escalation task lifecycle remain downstream WTBDs |
...
| Gold-standard conclusion | RFC-0041 has reached the expected manage-owned backend gold standard for explicit portfolio-list rebalance waves plus source-owned `PM_BOOK_REVIEW`, `CIO_MODEL_CHANGE`, bounded `RISK_EVENT` cohort discovery, bounded `TACTICAL_HOUSE_VIEW` cohort consumption through `lotus-advise` `TacticalHouseViewAffectedCohort:v1`, and bounded `BULK_REVIEW_CAMPAIGN` membership waves with optional governance evidence, persisted campaign definitions, campaign discovery, operating queue, read-only approval-attention inbox, read-only workflow board, read-only assignment plan, append-only approval-decision evidence, retirement/supersession lifecycle controls, lifecycle-event audit projection, workflow overview, readiness checks, launch packages, durable launch, and first-class launch-history audit pages. The bounded first-wave product path is implementation-backed across Gateway, Workbench, report/render/archive, and AI memo handoff, Gateway campaign-definition BFF composition is merged through `lotus-gateway` PR #212, and Workbench active campaign-definition list rendering is merged through `lotus-workbench` PR #184. Maker-checker workflow beyond append-only approval decisions, global campaign discovery, mutable assignment/escalation task lifecycle beyond the read-only plan, and external OMS execution remain unsupported future scope. |
...
lifecycle controls, lifecycle-event projection added as bounded audit posture, and workflow-board
projection plus assignment-plan projection added as read-only cross-actor operating posture.
Global campaign discovery, approval state mutation, maker-checker workflow, mutable assignment or
escalation task lifecycle beyond the read-only plan, and external execution remain explicit future
scope, not hidden defects in the supported wave path.
