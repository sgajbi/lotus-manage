...
| **Downstream Realization Depends On** | First-wave Gateway/Workbench wave command center, report/render/archive materialization, and AI PM memo handoff are implementation-backed; bounded manage risk-event consumption now uses `lotus-risk` `RiskEventAffectedCohort:v1`; bounded tactical house-view consumption now uses `lotus-advise` `TacticalHouseViewAffectedCohort:v1`; campaign/bulk-review membership is implemented as manage-owned `BulkReviewCampaignMembership:v1` over source-backed candidate portfolios with optional approval, expiry, actor-entitlement governance evidence, persisted `BulkReviewCampaignDefinition:v1` definitions, persisted `BulkReviewCampaignDiscovery:v1`, bounded campaign operating queue, bounded read-only approval-attention inbox, bounded read-only campaign workflow board, bounded read-only campaign assignment plan, bounded campaign workflow-automation readiness, append-only campaign assignment/escalation action ledger, campaign-definition retirement for definitions closed to new wave use, campaign-definition supersession for older versions replaced by active versions, campaign-definition lifecycle-event projection for bounded audit including launch history, campaign-definition preview-readiness checks for fail-closed new preview/create supportability, campaign-definition launch packages for bounded preview/create request and idempotency guidance, and durable campaign-definition launch when readiness is `READY`; RFC41-WTBD-003 is closed for Lotus-owned supportability, while external OMS execution, global portfolio-universe discovery, and external workflow orchestration remain explicit deferred owner dependencies and unsupported claims |
...
This RFC began as the pre-implementation execution guide and remains the authority record for
manage-owned rebalance-wave truth. Post-closure WTBD work has since delivered a bounded first-wave
product path: `lotus-gateway` composes wave command-center contracts, `lotus-workbench` renders the
wave command center, report/render/archive materialize governed wave reports, and `lotus-ai` owns
review-gated PM memo execution from bounded wave evidence.

The supported claim is deliberately bounded to those implemented flows. Gateway and Workbench
consume manage truth; they do not reconstruct wave state, source readiness, aggregate analytics,
...
`lotus-advise`, and `lotus-manage` consumes `TacticalHouseViewAffectedCohort:v1` for bounded
`TACTICAL_HOUSE_VIEW` wave preview/create over caller-supplied source-backed candidate portfolios;
...
owning source product. `TACTICAL_HOUSE_VIEW` requires a bank-authored tactical house-view payload,
source refs, caller-supplied candidate portfolios with source-owned portfolio type and
discretionary mandate posture, at least one eligible DPM portfolio type, and configured
`DPM_ADVISE_BASE_URL`; manage consumes `lotus-advise`
`TacticalHouseViewAffectedCohort:v1` and fails closed when the source authority is unavailable,
rejected, incomplete, or empty.
...
| Risk-event affected-cohort waves | Partial WTBD; bounded risk-event family is now manage-supported | `lotus-risk` owns `RiskEventAffectedCohort:v1` at `POST /analytics/risk/risk-event-cohorts/evaluate`, merged in `lotus-risk` PR #115 (`bd69d1576d8c01bdcfd2309202ef37f780cc2d06`), wiki-published as `91f933a`, and mirrored in `lotus-platform` PR #313 (`4218d4319d5dac82e87106429fadb14247c36515`). `lotus-manage` consumes it for `RISK_EVENT` preview/create with candidate portfolios, source-supplied exposure weights, fail-closed dependency handling, source-ref preservation, OpenAPI/API-vocabulary coverage, and repo-native consumer declaration validation. Tactical house-view source ownership is implemented in `lotus-advise` through `TacticalHouseViewAffectedCohort:v1`; Manage consumes that product for bounded `TACTICAL_HOUSE_VIEW` preview/create without recomputing house-view or portfolio facts. |
| Bulk-review campaign membership waves | Supported for bounded DPM operating campaigns | `lotus-manage` owns `BulkReviewCampaignMembership:v1` for `BULK_REVIEW_CAMPAIGN` preview/create, persisted `BulkReviewCampaignDefinition:v1` definitions at `/api/v1/rebalance/waves/campaign-definitions`, campaign-definition retirement at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/retire`, campaign-definition supersession at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/supersede`, lifecycle-event projection at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/lifecycle-events`, preview-readiness checks at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/preview-readiness`, launch packages at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/launch-package`, durable campaign launch at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/launch`, launch-history audit pages at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/launch-history`, append-only approval decisions at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/approval-decisions`, append-only assignment/escalation actions at `/api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/assignment-actions`, `BulkReviewCampaignDiscovery:v1` summaries at `/api/v1/rebalance/waves/campaign-discovery`, `BulkReviewCampaignOperatingQueue:v1` pages at `/api/v1/rebalance/waves/campaign-operating-queue`, and `BulkReviewCampaignApprovalInbox:v1` pages at `/api/v1/rebalance/waves/campaign-approval-inbox`. Manage requires candidate portfolios, source-owned `portfolio_type`, source refs, ISO as-of date, and eligible portfolio types; filters out non-DPM portfolios; emits trigger and item source refs with deterministic definition, discovery, membership, governance, readiness, launch-package, approval-decision, assignment-action, operating-queue, and approval-inbox content hashes where applicable; optionally preserves approval, expiry, access-purpose, source-ref, and actor-entitlement governance evidence; preserves source facts from core/risk/performance/advise refs without recalculating them; summarizes persisted campaign identity/governance/expiry/source-ref/candidate-count posture and emits `BulkReviewCampaignUniversePosture:v1` so clients receive a machine-readable no-global-portfolio-universe-discovery boundary; classifies persisted definitions as ready-to-launch, attention-required, or closed using existing discovery/readiness/lifecycle/launch-history posture; classifies approval attention as approval-complete, approval-required, approval-incomplete, expiry-attention, entitlement-attention, or closed using existing governance evidence and readiness posture; records append-only `APPROVED`, `REJECTED`, and `REQUIRES_REMEDIATION` campaign approval posture evidence on active definitions; records append-only `ASSIGNED`, `REASSIGNED`, `ESCALATED`, `DEESCALATED`, and `RESOLVED` assignment posture evidence on active definitions; preserves retired and superseded definitions for audit while blocking their use in new preview/create requests; records replacement version/content-hash lineage for superseded definitions; projects create/launch/retire/supersede audit events from the persisted definition record; returns bounded preview/create request drafts and idempotency/correlation header guidance; launches one durable wave only when readiness is `READY`; records and pages append-only launch history with wave id, actor, requested as-of date, correlation id, idempotency key, and no-order/no-OMS boundaries; and fails closed for missing candidate portfolios, missing portfolio type, missing source refs, invalid date, empty eligible portfolio-type filters, empty eligible membership, incomplete approval evidence, invalid or expired expiry dates, unauthorized actors, missing definitions, retired definitions, superseded definitions, missing/non-active supersession replacements, stale definition as-of date, blocked launch readiness, conflicting definition or lifecycle updates, conflicting approval decision refs, and conflicting assignment action refs. |
...
| Gateway wave composition | Supported through `lotus-gateway` PR #196 and PR #197 | Gateway composes manage wave truth only and must not calculate cohorts, classify source readiness, generate alternatives, approve/stage items, reconstruct proof packs, or claim execution. |
| Workbench wave command center | Supported through `lotus-workbench` PR #165 and PR #168 | Workbench renders Gateway/manage wave truth only and must not synthesize wave evidence, report input, AI memo content, PM recommendations, or external execution posture. |
...
9. Gateway/Workbench first-wave wave command-center realization is implemented, live-proven,
   merged, and wiki-published without moving wave authority out of manage,
...
| Gold-standard conclusion | RFC-0041 has reached the expected manage-owned backend gold standard for explicit portfolio-list rebalance waves plus source-owned `PM_BOOK_REVIEW`, `CIO_MODEL_CHANGE`, bounded `RISK_EVENT` cohort discovery, bounded `TACTICAL_HOUSE_VIEW` cohort consumption through `lotus-advise` `TacticalHouseViewAffectedCohort:v1`, and bounded `BULK_REVIEW_CAMPAIGN` membership waves with optional governance evidence, persisted campaign definitions, campaign discovery, operating queue, read-only approval-attention inbox, read-only workflow board, read-only assignment plan, bounded workflow-automation readiness, append-only approval-decision evidence, append-only assignment/escalation actions, controlled assignment-task lifecycle evidence, append-only maker-checker control evidence, retirement/supersession lifecycle controls, lifecycle-event audit projection, workflow overview, readiness checks, launch packages, durable launch, and first-class launch-history audit pages. The bounded first-wave product path is implementation-backed across Gateway, Workbench, report/render/archive, and AI memo handoff, Gateway campaign-definition BFF composition is merged through `lotus-gateway` PR #212 and downstream campaign workflow/audit realization is further backed by `lotus-gateway` PR #238, while Workbench active campaign-definition list rendering is merged through `lotus-workbench` PR #184 and downstream campaign workflow/audit rendering is further backed by `lotus-workbench` PR #340. RFC41-WTBD-003 is closed for Lotus-owned supportability; global portfolio-universe discovery, external workflow orchestration beyond Manage-side task readiness, and external OMS execution remain explicit deferred owner dependencies and unsupported claims. |

---

## 20. Post-Closure WTBD Integration Audit

The 2026-05-09 WTBD audit moved completed RFC41 work back into this RFC so the current
implementation truth is available in the owning RFC, not only in the WTBD ledger.

| WTBD | Integrated result | Current boundary |
| --- | --- | --- |
| RFC41-WTBD-001 | `lotus-core` PR #339 and `lotus-manage` PR #126 support source-owned `PortfolioManagerBookMembership:v1` PM-book wave discovery. | Risk-event and tactical house-view waves are supported through their separate source products; global campaign discovery and permission-denied cohort semantics remain future owner scope. |
| RFC41-WTBD-002 | `CioModelChangeAffectedCohort:v1` and manage `CIO_MODEL_CHANGE` preview/create support source-owned affected-mandate discovery. | Risk-event, tactical house-view, and bounded campaign membership waves are separate support claims; global campaign discovery remains unsupported. |
| RFC41-WTBD-003 | Completed for Lotus-owned supportability: `lotus-risk` PR #115 implements `RiskEventAffectedCohort:v1` and `lotus-manage` consumes it for bounded `RISK_EVENT` waves. `lotus-advise` owns `TacticalHouseViewAffectedCohort:v1` and `lotus-manage` consumes it for bounded `TACTICAL_HOUSE_VIEW` waves over source-backed candidate portfolios. `lotus-manage` now also implements bounded `BulkReviewCampaignMembership:v1` for `BULK_REVIEW_CAMPAIGN` preview/create over source-backed candidate portfolios with DPM portfolio-type filtering, deterministic membership source refs, optional approval/expiry/actor-entitlement governance evidence, persisted `BulkReviewCampaignDefinition:v1` definitions, persisted campaign discovery, campaign-definition retirement, campaign-definition supersession, lifecycle-event projection, workflow overview, preview-readiness checks, launch packages, durable launch, launch-history audit pages, append-only approval-decision evidence, append-only assignment/escalation actions, controlled assignment-task lifecycle evidence, bounded portfolio-memory assignment-task transition projection, append-only maker-checker control evidence, bounded operating queue, bounded read-only approval-attention inbox, bounded read-only workflow board, bounded read-only assignment plan, bounded read-only workflow-automation readiness, OpenAPI/API-vocabulary coverage, and fail-closed validation. `lotus-gateway` PR #212 exposes campaign-definition list/get/upsert BFF routes while preserving Manage truth and avoiding Gateway-local cohort or membership calculation. `lotus-workbench` PR #184 renders the active campaign-definition list through Gateway/BFF without browser-side cohort or membership calculation. | Global campaign discovery and external workflow orchestration beyond Manage-side task readiness are deferred owner dependencies represented by machine-readable unsupported boundary posture, not open Lotus-side implementation gaps. |
...
downstream product evidence for RFC41-WTBD-003 and supports the Lotus-owned closure posture:
global portfolio-universe campaign discovery and external workflow orchestration beyond Manage-side
task readiness remain deferred owner dependencies, and no order, OMS, fill, settlement, or
client-contact claim is promoted.
...
2. Existing manage wave proof remains anchored in
   `output/rfc0041-wave-proof/20260504-231914/manifest.json` and
   `output/rfc0041-wave-proof/20260507-224144/manifest.json`.
3. First-wave product proof remains anchored in
   ...
   `lotus-platform/output/front-office-qa/dpm-command-center-seed-20260507-143459.json`, and
...
`lotus-manage` now exposes `GET /api/v1/rebalance/waves/campaign-workflow-board` as a bounded
read-only `BulkReviewCampaignWorkflowBoard:v1` surface. It composes existing operating-queue and
approval-inbox posture into actor-aware next-action rows for launch, approval-decision capture,
approval evidence remediation, expiry refresh, entitlement review, attention review, or closed
posture. The builder is isolated in `src/core/waves/campaign_workflow_board.py`, so the router does
not duplicate readiness, approval, expiry, entitlement, launch-history, or lifecycle logic. This
narrows RFC41-WTBD-003's cross-actor workflow-surface gap without promoting global
portfolio-universe discovery, trade approval, order generation, external workflow orchestration,
client contact, or OMS execution.

2026-05-20 campaign assignment-plan addendum:

`lotus-manage` now exposes `GET /api/v1/rebalance/waves/campaign-assignment-plan` as a bounded
...
2026-05-20 campaign assignment-action addendum:
...
as a bounded append-only `BulkReviewCampaignDefinitionAssignmentActionPage` ledger. The domain
helper lives in `src/core/waves/campaign_assignment_actions.py` and records assignment,
...
contact clients, or claim OMS execution. RFC41-WTBD-003 is closed for Lotus-owned supportability;
global portfolio-universe campaign discovery and broader workflow automation beyond controlled
Manage-side assignment tasks and append-only evidence ledgers remain deferred owner dependencies
and unsupported claims.
...
membership, or claim OMS execution. RFC41-WTBD-003 is closed for Lotus-owned supportability; global
...
`lotus-manage` now exposes `GET /api/v1/rebalance/waves/campaign-workflow-automation` as a bounded
read-only `BulkReviewCampaignWorkflowAutomation:v1` surface. It composes
`BulkReviewCampaignAssignmentPlan:v1` with existing controlled assignment-task state to classify
automation candidates, manual-review rows, blocked rows, and closed rows. Candidate rows carry a
deterministic proposed task type and task ref for idempotent use of the existing assignment-task
...
hygiene confirming no stranded governance truth. The risk-event source-product foundation,
tactical house-view source-product consumption, and bounded manage campaign membership path are now
