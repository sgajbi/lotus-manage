...
| **Downstream Realization Depends On** | First-wave Gateway/Workbench wave command center, report/render/archive materialization, and AI PM memo handoff are implementation-backed; bounded manage risk-event consumption now uses `lotus-risk` `RiskEventAffectedCohort:v1`; bounded tactical house-view consumption now uses `lotus-advise` `TacticalHouseViewAffectedCohort:v1`; campaign/bulk-review membership is implemented as manage-owned `BulkReviewCampaignMembership:v1` over source-backed candidate portfolios with optional approval, expiry, actor-entitlement governance evidence, persisted `BulkReviewCampaignDefinition:v1` definitions, persisted `BulkReviewCampaignDiscovery:v1`, bounded campaign operating queue, bounded read-only approval-attention inbox, bounded read-only campaign workflow board, bounded read-only campaign assignment plan, append-only campaign assignment/escalation action ledger, campaign-definition retirement for definitions closed to new wave use, campaign-definition supersession for older versions replaced by active versions, campaign-definition lifecycle-event projection for bounded audit including launch history, campaign-definition preview-readiness checks for fail-closed new preview/create supportability, campaign-definition launch packages for bounded preview/create request and idempotency guidance, and durable campaign-definition launch when readiness is `READY`; external OMS execution, global portfolio-universe discovery, maker-checker workflow, and broader workflow automation remain downstream WTBDs |
