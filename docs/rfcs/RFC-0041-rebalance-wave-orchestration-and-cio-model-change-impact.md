...
| Bulk-review campaign membership waves | Supported for bounded DPM operating campaigns | `lotus-manage` owns `BulkReviewCampaignMembership:v1` for `BULK_REVIEW_CAMPAIGN` preview/create, immutable `BulkReviewCampaignDefinition:v1` definitions at `/api/v1/rebalance/waves/campaign-definitions`, and `BulkReviewCampaignDiscovery:v1` summaries at `/api/v1/rebalance/waves/campaign-discovery`. Manage requires candidate portfolios, source-owned `portfolio_type`, source refs, ISO as-of date, and eligible portfolio types; filters out non-DPM portfolios; emits trigger and item source refs with deterministic definition, discovery, membership, and governance content hashes where applicable; optionally preserves approval, expiry, access-purpose, source-ref, and actor-entitlement governance evidence; preserves source facts from core/risk/performance/advise refs without recalculating them; summarizes persisted campaign identity/governance/expiry/source-ref/candidate-count posture without discovering the global portfolio universe; and fails closed for missing candidate portfolios, missing portfolio type, missing source refs, invalid date, empty eligible portfolio-type filters, empty eligible membership, incomplete approval evidence, invalid or expired expiry dates, unauthorized actors, missing definitions, stale definition as-of date, and conflicting immutable definitions. |
...
| Gold-standard conclusion | RFC-0041 has reached the expected manage-owned backend gold standard for explicit portfolio-list rebalance waves plus source-owned `PM_BOOK_REVIEW`, `CIO_MODEL_CHANGE`, bounded `RISK_EVENT` cohort discovery, bounded `TACTICAL_HOUSE_VIEW` cohort consumption through `lotus-advise` `TacticalHouseViewAffectedCohort:v1`, and bounded `BULK_REVIEW_CAMPAIGN` membership waves with optional governance evidence and immutable campaign definitions. The bounded first-wave product path is implementation-backed across Gateway, Workbench, report/render/archive, and AI memo handoff, Gateway campaign-definition BFF composition is merged through `lotus-gateway` PR #212, and Workbench active campaign-definition list rendering is merged through `lotus-workbench` PR #184. Global campaign discovery, broader campaign workflow surfaces, and external OMS execution remain unsupported future scope. |
...
real and governed. Global campaign discovery, broader campaign workflow surfaces, and external
