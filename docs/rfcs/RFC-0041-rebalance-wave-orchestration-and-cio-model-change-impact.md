...
| Gold-standard conclusion | RFC-0041 has reached the expected manage-owned backend gold standard for explicit portfolio-list rebalance waves plus source-owned `PM_BOOK_REVIEW`, `CIO_MODEL_CHANGE`, bounded `RISK_EVENT` cohort discovery, bounded `TACTICAL_HOUSE_VIEW` cohort consumption through `lotus-advise` `TacticalHouseViewAffectedCohort:v1`, and bounded `BULK_REVIEW_CAMPAIGN` membership waves with optional governance evidence, persisted campaign definitions, campaign discovery, retirement/supersession lifecycle controls, lifecycle-event audit projection, readiness checks, launch packages, durable launch, and first-class launch-history audit pages. The bounded first-wave product path is implementation-backed across Gateway, Workbench, report/render/archive, and AI memo handoff, Gateway campaign-definition BFF composition is merged through `lotus-gateway` PR #212, and Workbench active campaign-definition list rendering is merged through `lotus-workbench` PR #184. Global campaign discovery, broader campaign workflow surfaces beyond bounded definition lifecycle controls, and external OMS execution remain unsupported future scope. |
...
| RFC41-WTBD-003 | Advanced again: `lotus-risk` PR #115 implements `RiskEventAffectedCohort:v1` and `lotus-manage` consumes it for bounded `RISK_EVENT` waves. `lotus-advise` owns `TacticalHouseViewAffectedCohort:v1` and `lotus-manage` consumes it for bounded `TACTICAL_HOUSE_VIEW` waves over source-backed candidate portfolios. `lotus-manage` now also implements bounded `BulkReviewCampaignMembership:v1` for `BULK_REVIEW_CAMPAIGN` preview/create over source-backed candidate portfolios with DPM portfolio-type filtering, deterministic membership source refs, optional approval/expiry/actor-entitlement governance evidence, persisted `BulkReviewCampaignDefinition:v1` definitions, persisted campaign discovery, campaign-definition retirement, campaign-definition supersession, lifecycle-event projection, preview-readiness checks, launch packages, durable launch, launch-history audit pages, OpenAPI/API-vocabulary coverage, and fail-closed validation. `lotus-gateway` PR #212 exposes campaign-definition list/get/upsert BFF routes while preserving Manage truth and avoiding Gateway-local cohort or membership calculation. `lotus-workbench` PR #184 renders the active campaign-definition list through Gateway/BFF without browser-side cohort or membership calculation. | Future global campaign discovery and broader campaign workflow surfaces beyond bounded definition lifecycle controls remain future depth beyond the first manage-owned campaign membership envelope, Gateway BFF composition, and Workbench active definition list. |
...
discovery, broader campaign workflow surfaces beyond bounded definition lifecycle controls, and
external execution remain explicit future scope, not hidden defects in the supported wave path.
