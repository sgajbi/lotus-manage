...
| Tax lots and realized tax | `lotus-core` or tax source owner | `TAX_OUTCOME` | Explicit transaction-row withholding tax adapter implemented. `lotus-core` also publishes source-owned `ClientTaxProfile:v1` and `ClientTaxRuleSet:v1` profile/rule evidence; portfolio-level realized-tax aggregation, tax-loss harvesting suitability, after-tax optimization, and jurisdiction-specific recommendations remain unsupported. | Consume source product; no manage-local tax-lot, client-tax, or tax-advice authority. |
...
| `TAX_OUTCOME` | `lotus-core` provides explicit transaction-row withholding tax evidence plus source-owned `ClientTaxProfile:v1` and `ClientTaxRuleSet:v1` profile/rule evidence where requested. Tax-lot allocation and portfolio-level realized-tax totals require future source-owner contracts, and tax advice, tax-loss harvesting suitability, after-tax optimization, and jurisdiction-specific recommendations remain unsupported. | `TAX_SOURCE_INCOMPLETE`. |
