...
| Cash movements | `lotus-core` | `CASH_OUTCOME` and MWR-related context | HoldingsAsOf cash totals, explicit linked transaction cashflow amounts, and PortfolioCashflowProjection total/booked/projected-settlement cashflow are implemented; liquidity ladders, client income-needs planning, and MWR-related flow context remain source-owner follow-on work. | Consume source product; missing source blocks cash outcome. |
...
| `CASH_OUTCOME` | `lotus-core` provides source-owned HoldingsAsOf cash total evidence, explicit linked transaction cashflow amounts, and PortfolioCashflowProjection total/booked/projected-settlement cashflow; liquidity ladders, client income-needs planning, and MWR flow context require future source-owner contracts. | `CASH_SOURCE_INCOMPLETE`. |
