...
| Cash movements and operational liquidity ladder | `lotus-core` | `CASH_OUTCOME` and MWR-related context | HoldingsAsOf cash totals, explicit linked transaction cashflow amounts, PortfolioCashflowProjection total/booked/projected-settlement cashflow, PortfolioCashMovementSummary bucket totals, and PortfolioLiquidityLadder opening-cash, bucketed cashflow, cumulative cash, shortfall, and asset-liquidity-tier exposure are implemented; client income-needs planning and MWR-related flow context remain source-owner follow-on work. | Consume source product; missing source blocks cash outcome. |
...
| `TAX_OUTCOME` | `lotus-core` provides explicit transaction-row withholding tax evidence plus source-owned `ClientTaxProfile:v1`, `ClientTaxRuleSet:v1`, and `PortfolioRealizedTaxSummary:v1` evidence where requested; Manage can preserve the portfolio summary total as source-owned realized-tax evidence without aggregating tax locally. Tax-lot allocation remains bounded to `PortfolioTaxLotWindow:v1`; tax advice, tax-loss harvesting suitability, after-tax optimization, jurisdiction-specific recommendations, client-tax approval, and tax-reporting certification remain unsupported. | `TAX_SOURCE_INCOMPLETE`. |
...
| `CASH_OUTCOME` | `lotus-core` provides source-owned HoldingsAsOf cash total evidence, explicit linked transaction cashflow amounts, PortfolioCashflowProjection total/booked/projected-settlement cashflow, PortfolioCashMovementSummary signed bucket totals, and PortfolioLiquidityLadder opening-cash, bucketed cashflow, cumulative-cash, shortfall, and asset-liquidity-tier exposure evidence; Manage can preserve source bucket totals without forecasting, funding advice, treasury instruction, or local cashflow aggregation. Client income-needs planning and MWR flow context require future source-owner contracts. | `CASH_SOURCE_INCOMPLETE`. |
