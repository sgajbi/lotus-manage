...
| `LIQUIDITY_AWARE` | Preserve funding, settlement, cash-buffer, and blocked-action posture. | Manage settlement engine plus core-sourced cash/liquidity fields where available. | Cash ladder, funding deficits, minimum cash buffer, and liquidity-policy context are in diagnostics. |
...
| `REGIME_STRESS_AWARE` | Compare construction against source-backed scenario-pack loss tolerance. | `lotus-risk` / CIO scenario pack authority context. | Scenario pack id, source system, worst-case loss, policy loss threshold, and reason codes are present; excess loss is pending review. |
...
| Liquidity and settlement | liquidity tier, settlement days/calendar, cash balances, cash ladder, minimum cash buffer | `lotus-core` for security liquidity and cash; manage derives settlement ladder and policy posture | Eligibility product plus existing settlement-aware engine | Use for liquidity-aware supportability, blockers, and pending-review posture. | Cashflow forecast and income need source products remain future depth; absence does not justify fabricating cashflow needs. |
...
| Regime/scenario | named scenario pack, shock vectors, stress result authority | `lotus-risk` / CIO scenario service | Manage accepts source-backed regime-stress authority context; no separate risk scenario endpoint exists yet | Use scenario pack id, worst-case loss, policy threshold, and reason codes to govern `REGIME_STRESS_AWARE`. | A first-class `lotus-risk` scenario-pack endpoint remains future upstream depth; manage must not generate scenario methodology locally. |
...
| `PortfolioCashflowForecast:v1` | `lotus-core` or cashflow source | income-need and cashflow-aware liquidity alternatives | Future depth for richer client cashflow planning; current `LIQUIDITY_AWARE` support uses settlement, funding, cash balances, and minimum cash policy. |
...
| `RegimeScenarioPack:v1` | `lotus-risk` / CIO scenario authority | regime-stress-aware construction | Future first-class upstream endpoint; current support requires explicit source-backed scenario-pack authority context and degrades when absent. |
