...
| `COST_AWARE` | Compare candidate construction using source-owned observed transaction-cost evidence. | `lotus-core` `TransactionCostCurve:v1`. | Observed average cost bps are applied to candidate trade notionals for comparison evidence only; missing or incomplete curve coverage degrades truthfully; predictive quotes and market-impact optimization are not claimed. |
...
| Mandate binding | mandate id/version, model binding, objective, risk profile, horizon, review cadence, review dates, tax-awareness allowed, rebalance bands, policy pack | `lotus-core` | `DiscretionaryMandateBinding:v1`; mandate digital twin derived in manage | Use for method eligibility, mandate objective, review-cycle health, policy selection, and trace context. **Not a source of mandate cash bands.** The binding carries `rebalance_bands.cash_reserve_weight`, an operating floor for rebalancing, and it must never be relabelled as a contractual cash-band minimum or maximum - deriving one from the other is the defect corrected under #664. | Mandate cash band, benchmark, and explicit turnover budget are all separate source gaps: no Core product states a contractual cash band today, so the mandate twin reports `MANDATE_CASH_BAND_NOT_YET_SOURCED` and `MANDATE_TURNOVER_BUDGET_NOT_YET_SOURCED` and keeps those health dimensions out of a READY assessment. |
...
| Transaction cost | observed average/min/max booked cost bps per security and transaction type | `lotus-core` source authority | `TransactionCostCurve:v1` through `/integration/portfolios/{portfolio_id}/transaction-cost-curve` | Use for `COST_AWARE` comparison evidence and RFC-0040 proof-pack turnover/cost supportability. `COST_AWARE` applies observed average bps to candidate trade notionals and records objective/constraint traces. | Predictive spread, market impact, venue routing, and true min-cost execution optimization remain future execution/source-owner methodology. |
...
| Regime/scenario | named scenario pack, shock vectors, stress result authority, optional reconciled per-security contribution rows | `lotus-risk` / CIO scenario service | `RegimeScenarioPackEvaluation:v1` through `POST /analytics/risk/regime-scenario-pack/evaluate`; manage still accepts caller-supplied source-backed regime-stress authority context | Use scenario pack id, worst-case loss, policy threshold, reason codes, and source-owned contribution rows when supplied to govern `REGIME_STRESS_AWARE`; when `DPM_RISK_BASE_URL` is configured, manage resolves the source product automatically for the method; proof-pack generation can preserve direct source-owned `regime_stress_context` when selected-alternative scenario context is absent. | Broader scenario assumptions, CIO approvals, portfolio/mandate applicability evidence, and product-surface display remain future depth; manage must not generate scenario methodology locally. |
...
| `objective_terms.estimated_cost` | Present for `COST_AWARE` when source curve coverage supports the candidate trades. | `lotus-core` observed `TransactionCostCurve:v1` bps applied to candidate trade notionals. | Must remain labelled comparison evidence, not a predictive quote; missing coverage degrades. |
| `constraints.cash_band` | Present for all alternatives. | Policy pack or request option. **Not the mandate binding**, which states no contractual cash band; its `cash_reserve_weight` is a rebalance operating floor and is not a band boundary. | Missing explicit band must be a local construction policy fallback carrying its own reason code, and must not be presented as a mandate constraint. A construction-local band and a contractual mandate band are different facts with different authorities. |
...
| `TransactionCostCurve:v1` | `lotus-core` source authority | proof-pack observed-cost evidence and `COST_AWARE` construction comparison | Implemented for observed-cost proof-pack evidence and cost-aware comparison metrics. Predictive execution quotes, market-impact modelling, venue routing, and true min-cost optimization remain future execution/source-owner work. |
...
| `RiskMetricsReport:v1`, `DrawdownAnalyticsReport:v1`, `HistoricalRiskAttribution:v1` | `lotus-risk` | risk-aware alternatives, tracking-error, drawdown, stress/attribution comparison context | Implemented as bounded source-owned analytics preservation. Manage preserves supplied context and blocks local risk methodology calculation. |
| `BenchmarkExposureContext:v1`, `ContributionAnalytics:v1`, `AttributionAnalytics:v1` | `lotus-performance` | benchmark-relative, contribution, attribution, and currency-attribution context | Implemented as bounded source-owned analytics preservation. Manage preserves supplied context and blocks local performance methodology calculation. |
| `RegimeScenarioPackEvaluation:v1` | `lotus-risk` / CIO scenario authority | regime-stress-aware construction | First-wave implemented. Manage consumes scenario pack id, source service, supportability, worst-case loss, policy threshold, breach reason codes, and source lineage from `POST /analytics/risk/regime-scenario-pack/evaluate` when `DPM_RISK_BASE_URL` is configured. `lotus-risk` PR #116 adds optional reconciled per-security `position_contributions` for each governed scenario when callers supply `exposure_components`; manage must preserve those rows when product flows request component-level scenario proof instead of recalculating scenario methodology locally. |
