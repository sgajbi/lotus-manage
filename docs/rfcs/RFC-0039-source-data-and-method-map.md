...
| Mandate binding | mandate id/version, model binding, objective, risk profile, horizon, review cadence, review dates, tax-awareness allowed, rebalance bands, policy pack | `lotus-core` | `DiscretionaryMandateBinding:v1`; mandate digital twin derived in manage | Use for method eligibility, mandate objective, review-cycle health, cash bands, policy selection, and trace context. | Benchmark and explicit turnover budget remain separate source gaps unless supplied by policy pack or later source products. |
