...
| --- | --- |
| Field-by-field source map exists | This document is the governed map for first-wave construction alternatives. |
...
| Method semantics are unambiguous before implementation | Each first-wave method has a bounded purpose, required data posture, comparison metrics, and fallback/degraded behavior. |
...
`SOLVER_CONSTRAINED` is implementation-eligible but not first-wave support-promoted until solver
trace, timeout, infeasibility, and deterministic fallback evidence are complete. Existing solver
target generation is useful implementation material, but not enough by itself to claim an advanced
...
| FX funding and settlement awareness | `src/core/rebalance/execution.py` and `EngineOptions.enable_settlement_awareness` | Reuse as comparison dimensions and blockers; do not promote `CURRENCY_OVERLAY` until hedge-specific data exists. |
...
| Liquidity and settlement | liquidity tier, settlement days/calendar, cash balances, cash ladder | `lotus-core` for security liquidity and cash; manage derives settlement ladder | Eligibility product plus existing settlement-aware engine | Use for liquidity diagnostics and blockers; first wave may compare liquidity posture but should not claim full liquidity optimization unless implemented. | Cashflow forecast and income need source products remain missing. |
...
| Risk context | tracking error, volatility, stress, concentration, drawdown, risk contribution | `lotus-risk` | Not currently integrated for RFC-0039 alternatives | Preserve enrichment seam and mark missing risk context. | Do not recompute authoritative risk. Local concentration/drift diagnostics must be labelled manage-side. |
...
| Currency overlay | strategic currency exposure, hedge-ratio bands, forward points, hedging eligibility, settlement evidence | `lotus-core` plus market-data/treasury source | FX spot coverage exists; hedge-specific data is absent | Existing FX funding may be compared as execution support, not a currency-overlay strategy. | `CurrencyExposurePolicy:v1`, hedge instrument eligibility, forward curves/points, and hedge-ratio bands are missing. |
| Regime/scenario | named scenario pack, shock vectors, stress result authority | `lotus-risk` / CIO scenario service | Not currently available | No first-wave support claim. | `RegimeScenarioPack:v1` or risk scenario endpoint required. |
...
| `PortfolioCashflowForecast:v1` | `lotus-core` or cashflow source | liquidity-aware alternatives | Required before full liquidity-aware support promotion. |
| `TransactionCostCurve:v1` | platform execution/cost source | cost-aware and min-cost alternatives | Local estimate allowed only as labelled diagnostic until this exists. |
| `CurrencyExposurePolicy:v1` | `lotus-core` / treasury policy source | currency overlay alternatives | Required before currency-overlay support promotion. |
| `RiskAlternativeEnrichment:v1` | `lotus-risk` | risk-aware alternatives, stress/drawdown/tracking-error comparison | Required before risk-aware support promotion. |
...
| `RegimeScenarioPack:v1` | `lotus-risk` / CIO scenario authority | regime-stress-aware construction | Required before regime-stress support promotion. |
