...
1. Report input carries review id, review hash, portfolio/mandate/run/proof/wave references,
   ...
   `DPM_OUTCOME_CLIENT_COMMUNICATION_BOUNDARY` evidence, redaction policy, evidence ref, and
   canonical content hash.
2. AI evidence input carries permitted use, forbidden actions, forbidden-field posture, bounded
   dimension facts, source refs, structured `DPM_OUTCOME_EXTERNAL_EXECUTION_BOUNDARY` and
   `DPM_OUTCOME_CLIENT_COMMUNICATION_BOUNDARY` evidence, evidence ref, and canonical content hash.
