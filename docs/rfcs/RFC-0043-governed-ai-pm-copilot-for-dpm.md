...
| Metadata | Details |
| --- | --- |
| **Status** | IMPLEMENTED - GOVERNED DPM WORKFLOW PACKS AND WORKBENCH COPILOT WORKSPACE |
| **Created** | 2026-05-03 |
...
2. `lotus-ai` owns the review-gated `dpm_pm_memo.pack@v1`, `dpm_wave_pm_memo.pack@v1`,
   ...
   `dpm_exception_summary.pack@v1`, and `pm_quality_summary.pack@v1` workflow-pack execution
   paths.
...
4. Gateway and Workbench have governed invocation and posture surfaces for the implemented DPM
   memo, operations handoff, exception-summary, PM-quality summary, and copilot workspace paths
   where recorded in the WTBD ledger.
...
8. `lotus-ai` PR #70 implements the owner-side PM quality summary pack over Manage-owned
   `PmOperatingQualityScoreRun` evidence.
9. `lotus-gateway` PR #209, `lotus-gateway` PR #210, `lotus-workbench` PR #182, and
   `lotus-workbench` PR #354 implement Gateway/Workbench invocation and the Gateway-only
   `mode=copilot` workspace across the supported pack families. Autonomous advice, PM ranking,
   client contact, order routing, OMS execution, raw prompt retention, generated model-output
   retention, and any additional future product-surface execution remain unsupported unless a
   future source-owner contract implements and proves them.
...
6. `pm_quality_summary.pack` - implemented as `pm_quality_summary.pack@v1` for review-gated
...
### 7.6 Workbench Copilot Workspace

Current implementation note:

`lotus-workbench` PR #354 implements the Gateway-only
`/workbench/{portfolioId}?mode=copilot` workspace for the supported DPM copilot families. The
workspace centralizes proof-pack PM memo, wave PM memo, outcome-review narrative, operations
handoff summary, exception summary, and PM-quality summary invocation posture while preserving
source refs, workflow run/artifact refs, readiness states, and unavailable boundaries. It does not
construct prompts, call Manage or AI directly, store generated summary text, rank PMs, contact
clients, approve trades, route orders, or claim OMS execution.
...
| Exception narrative | Supported for owner-side workflow-pack execution and product invocation | `lotus-ai` PR #68 implements `dpm_exception_summary.pack@v1` over bounded monitoring-exception evidence and optional portfolio-memory context with source refs, no-raw-payload posture, forbidden-action/output guardrails, review-required support-only output, and no client message, PM scoring, routing, approval, or execution claim. `lotus-gateway` PR #209 exposes the governed `/api/v1/dpm/command-center/exceptions/{exception_id}/ai-summary` route, and `lotus-workbench` PR #182 plus PR #354 expose Gateway-only exception-summary invocation/status display and copilot workspace realization. |
| Operations handoff summary | Supported for owner-side workflow-pack execution and product invocation | `lotus-ai` PR #67 implements `dpm_operations_handoff_summary.pack@v1` over Manage-owned `DpmWaveReportInput` handoff evidence with source refs, handoff refs, no-raw-payload posture, forbidden-action/output guardrails, review-required support-only output, and no external execution claim. `lotus-gateway` PR #210 exposes the governed `/api/v1/dpm/command-center/waves/{wave_id}/operations-handoff-summary` route, and `lotus-workbench` PR #182 plus PR #354 expose Gateway-only operations-handoff invocation/status display and copilot workspace realization. |
| PM quality summary | Supported for owner-side workflow-pack execution and product invocation | `lotus-ai` PR #70 implements `pm_quality_summary.pack@v1` over Manage-owned `PmOperatingQualityScoreRun` evidence with source refs, supportability posture, optional bounded portfolio-memory context, forbidden-action/output guardrails, review-required support-only output, and no score calculation, PM ranking, HR, compensation, conduct, client-contact, trade approval, execution, OMS, or invented-source-fact claim. `lotus-gateway` PR #239 and `lotus-workbench` PR #341 through PR #344 expose bounded summary-invocation list/detail/command/test-hardening realization, and Workbench PR #354 includes the PM-quality summary family in the Gateway-only copilot workspace. |
| Workbench copilot workspace | Supported for Gateway-only product realization across implemented pack families | `lotus-workbench` PR #354 implements `/workbench/{portfolioId}?mode=copilot` with list/detail/command/readiness/source-ref/run-posture rendering across proof-pack PM memo, wave PM memo, outcome-review narrative, operations handoff summary, exception summary, and PM-quality summary invocation paths. It preserves no direct Manage/AI calls, no browser prompt construction, no generated summary text retention, no PM ranking, no client contact, no order routing, and no OMS execution claim. |
| AI safety evaluation | Implemented for supported DPM pack families | Implemented DPM packs enforce forbidden-action, forbidden-field, unsupported-output, source-ref, no-raw-payload, and portfolio-memory source-authority guards. Additional future pack families or product surfaces must add owner-backed evaluation fixtures before support claims. |
...
| --- | --- |
| What was truly completed | DPM workflow-pack execution exists for proof-pack PM memo, wave PM memo, outcome-review narrative, operations handoff summary, exception summary, and PM quality summary; conservative workflow-pack default-version resolution is implemented in `lotus-ai`; Gateway exposes governed invocation routes; and Workbench now exposes a coherent Gateway-only `mode=copilot` workspace across supported pack families. |
| Quality improvements made | RFC-0043 now reflects actual `lotus-ai` workflow-pack ownership, completed Gateway/Workbench product realization for supported pack families, caller-side default-version clarity, and Gateway-only Workbench boundaries without leaving the copilot workspace as future-only wording. |
| Debt removed | Stale wording that treated every AI copilot feature or the full workspace as proposed was removed; default-version resolution, operations handoff summary support, exception summary support, PM-quality summary invocation, and the Gateway/Workbench copilot workspace are no longer stranded as future-only gaps. Unsupported autonomous advice, external execution, OMS acknowledgement, PM scoring, client messaging, raw prompt retention, generated output retention, and client-contact claims remain explicit. |
| What was proven through testing and evidence | The owning `lotus-ai` slices include guardrail tests for forbidden actions, forbidden fields, unsupported requested outputs, source refs, no-raw-payload posture, portfolio-memory lineage, review-gated run posture, and AI-owned source-event projections. `lotus-ai` PR #66 adds registry tests for default resolution, passes `make check`, passes Feature Lane and PR Merge Gate, and is wiki-published. `lotus-ai` PR #67 adds `dpm_operations_handoff_summary.pack@v1`, targeted guardrail/stub/API tests, full `make check`, live API proof for registry default, execution, mixed memo/handoff rejection, clean server logs, green Feature Lane and PR Merge Gate including coverage and Docker build, and wiki publication. `lotus-ai` PR #68 adds `dpm_exception_summary.pack@v1`, guardrail/stub/API tests for bounded monitoring-exception evidence, optional portfolio-memory context validation, forbidden client-message/PM-scoring/routing/action outputs, full local coverage proof at the 99 percent gate, green Feature Lane and PR Merge Gate including coverage and Docker build, merge to main, and wiki publication. `lotus-ai` PR #70 adds `pm_quality_summary.pack@v1`, guardrail/stub/API tests for Manage-owned `PmOperatingQualityScoreRun` evidence, optional portfolio-memory context validation, forbidden PM-ranking/HR/compensation/conduct/client-contact/trade-approval/execution/OMS outputs, full local `make ci`, green Feature Lane and PR Merge Gate including coverage and Docker build, merge to main, and wiki publication. `lotus-gateway` PR #209 and PR #210 add the governed Gateway routes with contract, unit, integration, mypy, `make check`, integration, coverage, Docker/parity CI, merge to main, and wiki publication. `lotus-workbench` PR #182 adds Gateway-only UI invocation and status display with focused Vitest coverage, typecheck, lint, docs guard, `make check`, build, Playwright smoke, Docker/parity CI, merge to main, and wiki publication. `lotus-workbench` PR #354 adds the Gateway-only `mode=copilot` workspace with focused navigation/component/API tests, `make lint`, `make typecheck`, `make test-coverage`, green Feature Lane, green PR Merge Gate, wiki publication commit `504a783`, and Main Releasability Gate `26347952758`. |
| Expected-standard decision | RFC-0043 reaches the expected standard for Lotus-owned governed AI PM copilot support: DPM workflow packs, default-version registry resolution, Gateway invocation routes, and Workbench `mode=copilot` workspace are merged, validated, and wiki-published. Additional future product surfaces remain owner-specific future scope and do not reopen RFC37-WTBD-002 unless they add new source-owner obligations. |
