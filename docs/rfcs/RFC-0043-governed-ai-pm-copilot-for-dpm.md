...
9. `lotus-gateway` PR #209, `lotus-gateway` PR #210, and `lotus-workbench` PR #182 implement
...
| PM quality summary | Supported for owner-side workflow-pack execution; product invocation remains future owner scope | `lotus-ai` PR #70 implements `pm_quality_summary.pack@v1` over Manage-owned `PmOperatingQualityScoreRun` evidence with source refs, supportability posture, optional bounded portfolio-memory context, forbidden-action/output guardrails, review-required support-only output, and no score calculation, PM ranking, HR, compensation, conduct, client-contact, trade approval, execution, OMS, or invented-source-fact claim. Gateway/Workbench invocation remains future scope and must consume governed Gateway/BFF routes only when implemented. |
