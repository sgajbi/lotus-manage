...
| Scenario pack authority | `lotus-risk` / CIO authority | Not required for original RFC-0040 closure; first-wave source now exists for selected RFC-0039 alternatives | `RegimeScenarioPackEvaluation:v1` through `POST /analytics/risk/regime-scenario-pack/evaluate` | Risk tests, manage adapter tests, and manage proof-pack preservation tests. | Proof packs preserve the selected alternative's source-backed scenario context; richer scenario contribution and CIO approval evidence remain future depth. |
