...
| Authoritative transaction-cost curve | `lotus-core` source authority consumed by `lotus-manage` | Yes for RFC-0040 proof-pack evidence; no for predictive execution quotes or min-cost optimization | `TransactionCostCurve:v1` through `/integration/portfolios/{portfolio_id}/transaction-cost-curve` | Core source-owner tests plus manage core-sourcing, construction, and proof-pack tests. | Implemented for proof-pack evidence preservation. Dedicated RFC-0039 cost-aware optimization remains future manage work. |
