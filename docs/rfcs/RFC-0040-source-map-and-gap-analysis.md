...
| `scenario_and_regime_evidence` | `lotus-risk` / CIO authority | Regime stress authority context accepted by manage | Capture source-backed scenario pack metadata where supplied. | `DEGRADED` until first-class risk/CIO scenario-pack endpoint exists. |
...
| Scenario pack authority | `lotus-risk` / CIO authority | No for first wave unless regime-stress support is promoted beyond supplied context | Risk-owned scenario-pack API | Risk tests and manage adapter tests. | Deferred beyond supplied source-backed context. |
