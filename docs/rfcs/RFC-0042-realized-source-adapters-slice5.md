...
6. Follow-on WTBD-006 core adapters in `src/core/outcomes/core_sources.py` that wrap the
   ...
   transaction-row scalar evidence, and `PortfolioCashflowProjection:v1` total, booked, and
   projected-settlement cashflow into RFC-0042 realized evidence without aggregating cash,
   transaction, tax, FX, or projection rows in manage.
7. Unit tests covering ready source evidence, missing execution evidence, missing risk/performance
   ...
   core transaction-row source wrapping, degraded transaction-ledger posture, and invalid core
   source payload rejection.
...
| --- | --- |
| `lotus-core` | Supported adapters consume `HoldingsAsOf:v1` cash total evidence for `CASH_RESIDUAL`, explicit `TransactionLedgerWindow:v1` transaction-row scalar evidence for `COST`, `TAX`, `FX_RESIDUAL`, and linked `CASH_RESIDUAL`, `PortfolioCashflowProjection:v1` total, booked, and projected-settlement cashflow for source-owned projected cash residual evidence, and `PortfolioLiquidityLadder:v1` operational opening-cash, bucketed-cashflow, cumulative-cash, shortfall, and asset-liquidity-tier exposure evidence. `TransactionLedgerWindow:v1` now also exposes optional reporting-currency restatement for row-level realized FX P&L local evidence. They preserve source product metadata, as-of date, generated/evidence timestamp, data-quality posture, source fingerprint, transaction id/type where applicable, projection range/include-projected posture where applicable, selected measure, and source currency. Missing or malformed evidence blocks affected dimensions. Aggregated transaction-window totals, execution/fill quality, client income-needs planning, and rule outcome adapters remain source-owner follow-on work until owning contracts expose source-owned totals. |
...
| `lotus-core` PortfolioCashflowProjection total/booked/projected-settlement cashflow source | `READY` when the source response includes product identity, portfolio id, as-of date, projection range, include-projected posture, portfolio currency, the selected source-emitted cashflow measure, and complete/ready data quality |
...
3. supported measures are the source-owned `total_net_cashflow`, `booked_total_net_cashflow`, and
...
7. missing selected cashflow measure, portfolio currency, projection range, or include-projected posture
...
`realized_transaction_source_from_transaction_ledger_response` accepts a validated `lotus-core`
`TransactionLedgerWindow:v1` response and emits a `DpmRealizedSourceSnapshot` for the selected
transaction-row scalar.

Implemented behavior:

1. source owner remains `lotus-core`,
2. source type is `TRANSACTION_LEDGER_WINDOW`,
3. the caller must select an explicit `transaction_id` and one supported measure:
   `trade_fee`, `withholding_tax_amount`, `realized_fx_pnl`, or `cashflow_amount`,
4. selected `trade_fee` maps to `COST`; selected `withholding_tax_amount` maps to `TAX`;
   selected `realized_fx_pnl` maps to `FX_RESIDUAL`; selected linked `cashflow_amount` maps to
   `CASH_RESIDUAL`,
5. reporting-currency fields are preserved when the source response includes them and a resolved
   `reporting_currency`; otherwise the source transaction currency/base currency is preserved,
6. `product_name`, `product_version`, `portfolio_id`, `as_of_date`, generated/evidence timestamp,
   data-quality status, source batch fingerprint, transaction id, transaction type, selected
   measure, selected source field, and source currency are preserved as lineage/supportability
   evidence,
7. incomplete, partial, stale, unavailable, or unknown source data quality degrades the source
   posture while preserving the source-owned value,
8. missing transaction rows or missing selected source values raise `CoreOutcomeSourceError` and
   cannot silently produce a ready outcome value.

Out of scope for this adapter:

1. aggregating transaction rows locally,
2. computing transaction-cost totals, realized tax, realized FX residuals, or cash-movement totals
   across a review window,
3. inferring execution quality, slippage, fill quality, or order acknowledgement,
4. converting currencies locally,
5. fabricating source fingerprints, transaction ids, evidence timestamps, or data-quality posture.
...
when a certified `PortfolioCashflowProjection:v1` total, booked, or projected-settlement cashflow
is supplied, and can mark
