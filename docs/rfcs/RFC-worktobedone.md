...
8. richer assignment, escalation, and maker-checker workflows define their control model before
   any mutable workflow claim is promoted beyond read-only board evidence.
...
bounded read-only workflow board, and Gateway BFF composition
...
decisions, and richer assignment/escalation workflow beyond the read-only board without moving
