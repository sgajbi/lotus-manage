...
| RFC-0040 | RFC40-WTBD-001 through RFC40-WTBD-010 are incorporated into `docs/rfcs/RFC-0040-pre-trade-proof-pack-and-evidence-fabric.md`, including the portfolio-memory source-event family posture that lists supported manage/report/AI/archive families, explicitly defers OMS execution, and supports bounded PM quality score-run lineage for persisted score runs whose source-owned Core PM-book membership evidence includes the requested portfolio. | CIO approval evidence, effective-period exception posture, direct proof-pack enrichment when no selected alternative carries scenario context, future OMS execution products, downstream UI, and richer score-run analytics remain future source depth rather than hidden manage proof-pack or portfolio-memory gaps. |
| RFC-0041 | RFC41-WTBD-001, RFC41-WTBD-002, RFC41-WTBD-004 through RFC41-WTBD-009, the bounded risk-event source-owner plus manage-consumer result for RFC41-WTBD-003, and the bounded manage-owned `BulkReviewCampaignMembership:v1` result are incorporated into `docs/rfcs/RFC-0041-rebalance-wave-orchestration-and-cio-model-change-impact.md`. | RFC41-WTBD-003 remains partial only for Manage tactical house-view consumption, global campaign discovery, and downstream campaign surfaces. Tactical house-view source ownership is implemented in `lotus-advise`: it owns `TacticalHouseViewAffectedCohort:v1` for governed bank-authored house-view instructions and caller-supplied source-backed candidates. `lotus-manage` now owns and implements the first `BulkReviewCampaignMembership:v1` DPM operating campaign envelope with optional approval, expiry, access-purpose, source-ref, actor-entitlement governance evidence, and immutable `BulkReviewCampaignDefinition:v1` definitions over source-backed candidate sets while source apps still own the underlying facts and calculated reasons. RFC41-WTBD-010 remains deferred execution scope with no supported claim. |
| RFC-0042 | RFC42-WTBD-001 through RFC42-WTBD-005 and the bounded RFC42-WTBD-008 PM operating quality policy administration, preview, persisted score-run lifecycle, governance controls, optional source-owned PM-book materialization, and bounded portfolio-memory score-run lineage projection are incorporated into `docs/rfcs/RFC-0042-post-trade-outcome-feedback-loop.md`. | RFC42-WTBD-006 remains partial source-family work; RFC42-WTBD-007 remains unsupported execution/OMS scope; RFC42-WTBD-008 remains partial for downstream UI and advanced cross-segment fairness analytics. |
...
| Done on merged/published truth | 43 | Implementation-backed items merged to owning `main` branches, validated, and published where wiki truth changed. |
| Partial / in progress | 4 | Items with meaningful implementation-backed progress but known source-owner or downstream gaps. |
| Remaining / open | 12 | Items still deferred, proposed, conditional, unsupported, or awaiting ownership. |
...
| 2 | RFC41-WTBD-003 - Tactical house-view, risk-event, and campaign/bulk-review cohorts | Moves the rebalance wave operating model toward bank operating workflows without inventing source-owned cohorts. Risk-event source ownership, bounded manage consumption, first-wave Manage-owned bulk-review campaign membership with optional governance evidence, immutable Manage-owned campaign definitions over source-backed candidate sets, and Advise-owned `TacticalHouseViewAffectedCohort:v1` source ownership are now implemented. | Manage tactical house-view consumption, global discovery, and downstream campaign product surfaces are added only after source behavior and product evidence are proven. |
...
| RFC37-WTBD-002 | Complete RFC-0043 governed AI PM copilot | `lotus-ai`, consumed by Gateway/Workbench/manage evidence | Completed for the bounded current product path; partial for the broader copilot workspace | `lotus-ai` now owns review-gated proof-pack PM memo, wave PM memo, outcome-review narrative, operations handoff summary, and exception summary packs with guardrails and provenance, plus conservative workflow-pack default-version resolution. Gateway and Workbench now expose the operations handoff and exception-summary invocation surfaces through governed Gateway routes. Full copilot workspace and any additional product surfaces remain future work. |
...
| Gold-pass question | Assessment |
| --- | --- |
| What was truly completed | DPM workflow-pack execution exists for proof-pack PM memo, wave PM memo, outcome-review narrative, operations handoff summary, and exception summary; `lotus-ai` exposes conservative default-version resolution; Gateway and Workbench now expose the first-wave operations handoff and exception-summary invocation surfaces through governed Gateway routes. |
...
| RFC39-WTBD-007 | Cashflow/income-need aware liquidity construction | `lotus-core` source authority consumed by `lotus-manage` | Ownership decision made; income-needs and liquidity-reserve source products remain pending | `LIQUIDITY_AWARE` now accepts `lotus-core` `PortfolioCashflowProjection:v1` total net cashflow evidence and evaluates projected cash pressure against minimum cash policy. Future client-income support is decided for `lotus-core` through `ClientIncomeNeedsSchedule:v1`, `LiquidityReserveRequirement:v1`, and optional `PlannedWithdrawalSchedule:v1`, potentially ingested from external bank planning/reference systems. Manage may consume these products for liquidity-aware construction, mandate health, proof packs, waves, and outcome review, but must not claim financial-planning advice, funding recommendation, or client liability planning. |
| RFC39-WTBD-008 | External treasury/currency overlay source boundary | `lotus-core` ingestion of external bank treasury/currency products, consumed by `lotus-manage` | Ownership decision made; Lotus-side external-source contracts and consumption remain pending | Current support uses FX readiness and bounded currency-overlay context. Lotus will not introduce a `lotus-treasury` app; bank treasury systems should map into Lotus-side contracts for external currency exposure, hedge policy, FX forward curves, eligible hedge instruments, and hedge execution readiness. Manage may consume that evidence for construction/rebalance posture but must not price forwards, approve hedge policy, choose counterparties, or claim hedge execution. |
...
Implement Lotus-side external treasury/currency contracts and `lotus-core` ingestion first, then
upgrade manage `CURRENCY_OVERLAY` diagnostics and supported-feature claims to consume the products
with fail-closed supportability. Bank deployment maps local treasury systems into those contracts.
...
`REGIME_STRESS_AWARE` alternatives. RFC-0040 selected-alternative proof packs now preserve that
source context in `scenario_and_regime_evidence` when the selected construction alternative carries
it, including `RegimeScenarioPackEvaluation` source refs, canonical `regime_stress_context` source
hashes, scenario pack id, worst-case loss, maximum allowed loss, supportability state, and bounded
reason codes. The `lotus-risk` source product now also supports optional reconciled
`exposure_components` and emits per-security `position_contributions` for each governed scenario
when callers supply those components.

What remains deferred:

RFC-0040 deliberately does not call `lotus-risk` directly to enrich proof packs that lack selected
construction scenario evidence. It also does not yet receive CIO approval workflow evidence,
effective-period exception posture, or portfolio/mandate applicability evidence beyond the
first-wave evaluation reason codes and optional risk-owned contribution rows.

Remaining dependencies before full proof-pack enrichment:

1. CIO approval and applicability evidence from `lotus-risk` / CIO authority if product claims
   require it,
...
The first-wave source and selected-alternative preservation path is implemented and audited.
Direct proof-pack enrichment and richer CIO approval/applicability evidence should be handled only
if a future proof-pack/product RFC requires those fields and the source owner exposes the required
product.
...
2. `tests/unit/dpm/proof_packs/test_proof_pack_builder.py` covers source preservation,
   supportability state mapping, source refs, source hashes, and malformed context rejection,
3. documentation and supported-feature updates distinguish selected-alternative preservation from
   future direct enrichment and scenario approval evidence.
...
| Gold-pass question | Assessment |
| --- | --- |
| What was truly completed | Manage proof packs preserve selected-alternative `RegimeScenarioPackEvaluation:v1` context in `scenario_and_regime_evidence` instead of degrading when the chosen alternative already carries source-owned regime-stress context. `lotus-risk` now also owns optional reconciled per-security scenario contribution rows in the same source product. |
| Quality improvements made | Scenario/regime proof-pack handling now follows the same source-owned analytics pattern as risk, performance, transaction cost, client restriction, and sustainability preference evidence. The source product now separates aggregate scenario posture from optional component-level contribution evidence without moving scenario methodology into manage. |
...
| What was proven through testing and evidence | Focused proof-pack tests passed and now assert source refs, source hashes, bounded scenario facts, scenario metrics, reason codes, degraded fallback behavior, and malformed-context rejection. `lotus-risk` PR #116 passed Feature Lane and PR Merge Gate checks, local live API proof for contribution rows, negative 422 validation for unreconciled components, risk log review, and canonical Workbench validation for `PB_SG_GLOBAL_BAL_001`. |
| Expected-standard decision | The manage-owned part of RFC40-WTBD-009 reaches the expected standard for selected-alternative evidence preservation, and the risk-owned scenario contribution source product reaches the expected standard for optional reconciled contribution rows. CIO approval workflow, effective-period exception posture, portfolio/mandate applicability, and direct proof-pack enrichment without selected-alternative scenario context remain future source-owner/product depth. |
...
| RFC41-WTBD-003 | Tactical house-view, risk-event, and campaign/bulk-review cohorts | CIO/risk/campaign source owners, with `lotus-risk` owning the first risk-event cohort source product, `lotus-advise` owning the first tactical house-view cohort source product, and `lotus-manage` owning the first campaign membership envelope | Partial: `lotus-risk` `RiskEventAffectedCohort:v1` is merged, CI-proven, wiki-published, and mirrored in platform mesh governance; `lotus-manage` consumes it for `RISK_EVENT` preview/create. `lotus-advise` `TacticalHouseViewAffectedCohort:v1` is merged, CI-proven, and wiki-published as source-owned affected-cohort evidence. `lotus-manage` now also supports bounded `BULK_REVIEW_CAMPAIGN` preview/create through `BulkReviewCampaignMembership:v1` over source-backed candidate portfolios with optional approval, expiry, and actor-entitlement governance evidence. | Manage tactical house-view consumption, global campaign discovery, and downstream product surfaces remain future depth. |
...
persist campaign definitions, run a maker-checker workflow, or claim downstream Workbench campaign
UI support.

The broader WTBD remains partial because tactical house-view is not yet implemented and richer
campaign controls remain future depth. The tactical house-view source-owner decision is explicit:
`lotus-advise` should publish a governed bank-wide `TacticalHouseViewAffectedCohort:v1` as a
materialized advisory/CIO policy data product over `lotus-core` portfolio, mandate, holding, and
restriction snapshots, with optional `lotus-risk` exposure evidence. `lotus-manage` should consume
that product only for DPM/discretionary portfolios and must preserve source refs, effective-period
posture, approval posture, and supportability without interpreting CIO policy locally. Future
campaign depth should expand the now-promoted manage-owned membership envelope with persisted
campaign definitions, global discovery, and downstream product-surface behavior only after those
controls are proven.
...
4. richer campaign cohorts define persisted campaign definition and global discovery governance,
5. manage validates supportability without calculating underlying source facts locally.

Expected remaining implementation wave:

Treat tactical house-view and richer campaign controls as separate RFCs or explicit slices.
Tactical house-view should start in `lotus-advise` with `TacticalHouseViewAffectedCohort:v1`.
Campaign/bulk-review now has a first promoted manage-owned membership envelope with optional
approval, expiry, and actor-entitlement governance evidence; future slices should add persisted
campaign definitions, global discovery, and downstream product surfaces without moving source-fact
ownership into Manage.
...
| Expected-standard decision | The bounded `RISK_EVENT` manage-consumer slice reaches the expected backend standard on merged `lotus-manage` `main` truth with green GitHub checks and synchronized repo-local wiki publication. The bounded `BULK_REVIEW_CAMPAIGN` membership slice is implementation-backed on `main`; this governance-evidence slice adds approval, expiry, and actor-entitlement controls. RFC41-WTBD-003 remains partial for tactical house-view, persisted campaign definitions, global discovery, and downstream campaign product surfaces. |
...
| Expected-standard decision | This reaches the backend standard for bounded governance evidence on Manage-owned campaign membership. Tactical house-view, persisted campaign definitions, global campaign discovery, downstream Gateway/Workbench campaign UI, and OMS execution remain outside the support claim. |
...
This completion does not promote tactical house-view, risk-event, campaign cohort discovery, or
external OMS execution. Those remain separate source-owner or execution-product WTBDs.
...
11. no broader methodology is claimed yet: aggregated tax, aggregated FX, aggregated
   transaction-cost, execution, client income-needs planning,
...
Latest WTBD-006 client tax-profile and tax-rule ownership decision:

1. `lotus-core` is the Lotus-side owner for future `ClientTaxProfile:v1` and
   `ClientTaxRuleSet:v1` products because client/portfolio reference data, booking jurisdiction,
   tax lots, transaction evidence, and source-readiness composition already belong in Core,
2. deployed banks may map external tax/reference systems into those Core products; Lotus does not
   need a separate tax application for the current WTBD decision,
3. `lotus-manage` may consume these products for bounded tax-aware construction, proof-pack, and
   ...
   recommendation,
4. implementation remains pending until Core publishes contracts, supportability states, examples,
   tests, and source-owner methodology/docs for the profile and rule-set products.
...
source-owned PM-book materialization, and bounded portfolio-memory score-run lineage projection.
