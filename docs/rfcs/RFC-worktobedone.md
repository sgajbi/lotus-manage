...
| 2 | RFC41-WTBD-003 - Tactical house-view, risk-event, and campaign/bulk-review cohorts | Moves the rebalance wave operating model toward bank operating workflows without inventing source-owned cohorts. Risk-event source ownership, bounded manage consumption, first-wave Manage-owned bulk-review campaign membership with optional governance evidence, and immutable Manage-owned campaign definitions over source-backed candidate sets are now implemented. Tactical house-view ownership is decided for `lotus-advise`. | `lotus-advise` publishes `TacticalHouseViewAffectedCohort:v1`; global discovery and downstream campaign product surfaces are added only after source behavior and product evidence are proven. |
...
| RFC41-WTBD-003 | Tactical house-view, risk-event, and campaign/bulk-review cohorts | CIO/risk/campaign source owners, with `lotus-risk` owning the first risk-event cohort source product and `lotus-manage` owning the first campaign membership envelope | Partial: `lotus-risk` `RiskEventAffectedCohort:v1` is merged, CI-proven, wiki-published, and mirrored in platform mesh governance; `lotus-manage` consumes it for `RISK_EVENT` preview/create. `lotus-manage` now also supports bounded `BULK_REVIEW_CAMPAIGN` preview/create through `BulkReviewCampaignMembership:v1` over source-backed candidate portfolios with optional approval, expiry, and actor-entitlement governance evidence. | Tactical house-view ownership is decided for `lotus-advise`, which should publish bank-wide `TacticalHouseViewAffectedCohort:v1` over core portfolio/mandate/holding/restriction snapshots and optional risk exposure evidence. Persisted campaign definitions, global campaign discovery, and downstream product surfaces remain future depth. |
