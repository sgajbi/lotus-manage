...
| RFC-0037 | RFC37-WTBD-001, RFC37-WTBD-002, RFC37-WTBD-005, RFC37-WTBD-006, and RFC37-WTBD-007 are incorporated into `docs/rfcs/RFC-0037-dpm-operating-system-and-mandate-intelligence.md` as completed parent-roadmap closure results. | RFC-0037 remains a strategic parent roadmap; broader strategic source-product depth, broader front-office realization, and additional future copilot product surfaces stay outside these completed parent-roadmap claims. Outcome-review source-methodology follow-ons remain tracked by RFC42-WTBD-006; external OMS/client-communication promotion dependencies remain explicit non-claims rather than RFC37-WTBD-001 or RFC37-WTBD-007 blockers. |
...
WTBD count semantics: this snapshot counts Lotus-owned supportability completion. A WTBD can move
to done when the Lotus-owned source contract, fail-closed posture, consumer declaration, lineage,
supportability evidence, non-claim boundaries, documentation, and publication proof are merged and
validated, even if a deployed bank-owned system still must supply production data before the
capability can be promoted from blocked boundary evidence to ready operational support. External
owner dependencies remain explicit in the row and do not become hidden support claims. When the only
remaining dependency is outside Lotus apps, the row is closed for Lotus-owned supportability with
explicit external-owner non-claims rather than left open as Lotus implementation work. WTBDs with
...
workflow-boundary, portfolio-memory, source-event posture, search-level no-OMS/no-client-contact
boundary evidence, and downstream realization support while representing global universe, broader
cross-app source-event search, bank-owned OMS events, client communication events, and external
workflow work as deferred owner dependencies. The earlier 2026-05-22 recount moved
RFC38-WTBD-007, RFC39-WTBD-008, and
RFC42-WTBD-007 out of partial because Lotus has implemented bounded
...
external-owner promotion dependencies, not blockers for Lotus-owned WTBD completion. RFC41-WTBD-010
is also closed for Lotus-owned supportability because the remaining execution/OMS integration is an
external-owner dependency and Lotus already emits fail-closed wave/report-input boundary evidence
without claiming order routing, acknowledgement, fill, settlement, reconciliation, or execution.
RFC36-WTBD-004 is now closed for Lotus-owned mesh-certification promotion because
...
review-action BFF composition is merged and mainline validated, and Workbench read-only
review-action ledger/detail rendering plus preview-before-create command UX are merged and
mainline validated. Approval workflow beyond immutable review actions remains future scope.
`lotus-performance` PR #168 (`781415f`, wiki `6fb7209`) and PR #170 (`5eca912`, wiki
`ffc69f8`) are now part of the closed RFC42-WTBD-006 Lotus-owned source-methodology supportability
claim. They add source-owned MWR source-preconverted FX evidence and single-currency no-conversion
posture without making Manage calculate FX, tax, execution, or OMS methodology locally. Broader FX
claims outside implemented performance/core paths, predictive execution, and OMS remain unsupported
non-claims rather than open Lotus-owned methodology blockers.
Gateway PR #242 (`ff4013f`) and Workbench PR #350 (`4e515a3`) backfill downstream realization for
already-supported Manage source-lineage search/facet truth: Gateway forwards bounded
portfolio-memory and outcome-review `source_system`, `source_type`, and `source_scan_limit`
filters while preserving Manage facets and support boundaries, and Workbench renders those
Gateway-only facets and applied filters. This supports the closed RFC40-WTBD-010 portfolio-memory
posture and the closed RFC42-WTBD-006 source-methodology supportability posture without promoting
broader cross-app event search, source-owner store queries, or local methodology calculation.
...
The RFC39-WTBD-005 construction risk/performance context slice moves that row from open to partial:
...
The RFC37-WTBD-001 closure slice moves that parent row to done for the Lotus-owned RFC-0042
outcome-feedback loop: Manage authority, Gateway/Workbench product realization, report/render/
archive materialization, governed AI narrative request flow, PM operating quality linkage,
source-lineage search/facets, and structured external OMS/client-communication boundaries are all
merged in their owning repositories. Richer source methodology depth remains in RFC42-WTBD-006,
while bank-owned OMS/client-communication runtime promotion remains an explicit external dependency
and unsupported claim rather than a blocker for the Lotus-owned outcome-loop WTBD.
...
preserves caller-supplied source context refs plus health-state/threshold posture without
calculating risk or performance methodology.
...
deferred source-owner products rather than hidden portfolio-memory functionality. Campaign
assignment-task transitions are now projected as bounded no-raw-rationale portfolio-memory source
...
This contributed to the now-closed RFC42-WTBD-006 Lotus-owned source-methodology supportability
claim without adding any Manage-local FX attribution calculation, tax, execution, OMS, or
...
RFC41-WTBD-003 downstream product evidence. The WTBD is now closed for Lotus-owned supportability: global
portfolio-universe campaign discovery is a deferred source-owner promotion dependency, external workflow orchestration beyond Manage-side task
readiness is an external-owner promotion dependency, and no order, OMS, fill, settlement, or client-contact
claim is promoted.
Downstream campaign-definition realization evidence includes `lotus-gateway` PR #212 (`891d00dea0328525ada14fabad278af1d1b9386b`),
PR #238 (`72c1397`) adds bounded campaign workflow/audit BFF routes, `lotus-workbench` PR #184 (`502aa2113a6e3954d53a35f9f956d258c84d5be0`), published Workbench wiki commit `694d9cd`, and
PR #340 (`ec9bcd7`) renders bounded campaign workflow/audit evidence. External workflow orchestration beyond Manage-side task readiness is an external-owner dependency; external workflow orchestration beyond Manage-side task readiness is not a Manage-owned workflow claim.
...
Completed for Lotus-owned DPM source-readiness mesh promotion. `lotus-core:DpmSourceReadiness:v1`
now has repo-native trust telemetry and `lotus-platform` promotes it into the RFC-0091 maturity
wave with SLO, access, evidence-pack, generated maturity matrix, context, wiki, and cross-repo mesh
certification proof. This closes the WTBD for the additional Lotus-owned stateful DPM
source-product certification path without claiming that every possible future DPM source family has
been promoted.

Implemented dependencies:

1. `lotus-core` owns and declares `DpmSourceReadiness:v1`,
2. `lotus-manage` declares the DPM source-readiness consumer dependency,
3. `lotus-core` PR #377 adds `DpmSourceReadiness:v1` trust telemetry,
4. `lotus-platform` PR #341 adds SLO/access/evidence policies and maturity-wave scope,
5. cross-repo mesh certification passes with the seven-product required set.
...
5. `lotus-core` PR #377 (`f1e5180`) adds `DpmSourceReadiness:v1` trust telemetry and
   `lotus-platform` PR #341 (`9d00b81`) promotes it into the RFC-0091 maturity wave with policies
   and generated matrix proof.
...
| RFC37-WTBD-007 | Portfolio memory across mandate, construction, proof-pack, wave, outcome, report, AI, and generated-document events | Cross-app, with manage as workflow/evidence participant | Completed for Lotus-owned portfolio-memory supportability | Manage/Gateway/Workbench portfolio memory, bounded Manage-local `GET /api/v1/rebalance/portfolio-memory/search` with event-type, supportability-state, represented source-system, represented source-type, matching-event facet, stable event lookup support, search-level source-event family posture, search-level external execution boundary evidence, and search-level client communication boundary evidence, persisted construction alternative set and selected-alternative lineage, report-owned source events, AI workflow-pack source events, archive generated-document/client-delivery source events, bounded PM quality score-run, review-action, and summary-invocation lineage, explicit fail-closed `ExternalOrderExecutionAcknowledgement:v1` deferred source-event posture, `DPM_PORTFOLIO_MEMORY_EXTERNAL_EXECUTION_BOUNDARY` evidence, and `DPM_PORTFOLIO_MEMORY_CLIENT_COMMUNICATION_BOUNDARY` evidence exist. Full bank-owned OMS execution/acknowledgement/fill/settlement event projection, client communication event projection, global portfolio-universe discovery, and broader cross-app source-event search/discovery are unsupported external-owner or future source-owner promotion dependencies, not remaining Manage-owned WTBD gaps. |
...
RFC37-WTBD-001 is now complete for the Lotus-owned outcome-feedback loop because the RFC-0042
first-wave product path is implemented across its owning repositories: Manage is the outcome-review
authority, Gateway and Workbench realize the product path, report/render/archive materialize
governed artifacts, AI owns the guarded outcome narrative workflow, PM operating quality consumes
outcome-review evidence under governed policy, and source-lineage filters/facets expose represented
source families without recalculation.

Remaining boundaries:

Richer realized risk/performance/tax/FX/cash methodology depth remains tracked by RFC42-WTBD-006.
External OMS execution, fill, settlement, and client-communication runtime support remain explicit
external-owner promotion dependencies and unsupported product claims. They are not blockers for
closing the Lotus-owned RFC37-WTBD-001 parent outcome-loop WTBD.

Future promotion dependencies:
...
3. external OMS execution and client communication runtime promotion remain unsupported external-
   owner dependencies, and richer source-owner methodology depth remains out of the demo story until
...
`ExternalOrderExecutionAcknowledgement:v1` source product, deterministic content hash, and
promotion requirements for certified OMS source ownership, source-product contracts,
lineage/freshness, acknowledgement/fill/settlement reconciliation, Manage consumer declaration,
Gateway/Workbench realization, and operations audit evidence; portfolio memory does not project
acknowledgement, fill, settlement, or execution-status events until bank-owned OMS acknowledgement
ingestion publishes a certified no-raw-payload source-event family.

The read model also names `client_communication` as a deferred source-event family and emits
structured `DPM_PORTFOLIO_MEMORY_CLIENT_COMMUNICATION_BOUNDARY` evidence with blocked client
contact, message generation, delivery confirmation, client approval, and communication-audit
capabilities, required future client-communication owner, required `ClientCommunicationRecord:v1`
source product, deterministic content hash, and promotion requirements for certified
client-communication source ownership, delivery/approval/audit reconciliation, downstream
realization, and consent/evidence controls. Portfolio memory does not project client-contact,
client-message, client-delivery, client-approval, or communication-audit events until a certified
source owner publishes governed no-raw-payload client communication source events.

Closure decision:

RFC37-WTBD-007 is complete for the current Lotus-owned bank-buyable support claim. Manage now
exposes detail and search-level source-event family posture, deterministic event identity,
retention/redaction/access/audit policy, bounded Manage-local source-lineage search, stable event
lookup, report/AI/archive/PM-quality lineage projection, and structured fail-closed external
execution and client communication boundary evidence. Gateway preserves the Manage payload through
its command-center BFF, and Workbench has bounded source-lineage search/facet realization.

Remaining bank-owned OMS acknowledgement/fill/settlement event projection, governed client
communication source events, global portfolio-universe discovery, and broader cross-app
source-event discovery are unsupported external-owner or future source-owner promotion
dependencies. They are not open Manage implementation gaps because Lotus must not fabricate those
events from supportability posture.
...
| RFC38-WTBD-007 | Broader risk and performance health enrichment | `lotus-risk`, `lotus-performance`, consumed by `lotus-manage` | Completed for bounded source-owned health-context preservation and Manage-side mandate-health consumption | `lotus-risk:MandateRiskHealthContext:v1` and `lotus-performance:MandatePerformanceHealthContext:v1` are source-owned products. Manage mandate-health recalculation accepts bounded caller-supplied source health contexts, validates product identity and request fingerprints, preserves source context refs in `source_analytics_posture`, and uses source-owned health-state/threshold posture for `RISK_DRIFT` and `PERFORMANCE_ATTENTION` without calculating tracking error, volatility, drawdown, attribution, benchmark-relative performance, FX, or other analytics methodology locally. |
...
Manage mandate-health snapshots now emit typed `source_analytics_posture` evidence for
risk/performance health context. The posture records supplied tracking-error and
performance-attention signals, records supplied source health contexts, preserves stable
`lotus-risk:MandateRiskHealthContext:v1:<request_fingerprint>` and
`lotus-performance:MandatePerformanceHealthContext:v1:<request_fingerprint>` refs, and lists
blocked local-calculation capabilities for tracking error, volatility, drawdown, attribution, and
benchmark-relative performance. When source health contexts are supplied, Manage uses only the
source product `health_state` and `threshold_breached` posture for `RISK_DRIFT` and
`PERFORMANCE_ATTENTION`. Manage does not calculate or clone risk or performance methodology.

Completion boundary:

1. `lotus-risk` owns `MandateRiskHealthContext:v1`, including tracking-error methodology,
   benchmark context, lineage, request fingerprint, source services, and upstream request
   fingerprints.
2. `lotus-performance` owns `MandatePerformanceHealthContext:v1`, including performance
   attention methodology, benchmark context, lineage, request fingerprint, and source services.
3. `lotus-manage` validates the bounded product identity and preserves context refs in mandate
   health snapshots, but does not become source authority for risk or performance analytics.
4. Gateway/Workbench profile-detail presentation remains downstream product depth rather than a
   blocker for Lotus-owned WTBD completion.

Promotion proof:

1. owning-service API certification for `MandateRiskHealthContext:v1` and
   `MandatePerformanceHealthContext:v1`,
2. manage mandate-health domain and API tests for context preservation and source-health scoring,
3. consumer contract declaration for both source products,
4. supported-feature and wiki updates naming bounded source-context support and methodology
   non-claims.
...
This closes RFC39-WTBD-008 for Lotus-owned supportability: the source contracts, fail-closed Core
postures, Manage consumer preservation, lineage, hashes, blocked-capability evidence, and
non-claims are implemented and published. It does not certify an external treasury ingestion table
or add FX attribution, hedge-policy approval, eligible-instrument selection, suitability approval,
product recommendation, hedge advice, forward pricing, FX valuation methodology, treasury
instruction, counterparty selection, best execution, OMS acknowledgement, fills, settlement, or
autonomous treasury action. Those runtime treasury feeds remain external bank-owner promotion
dependencies rather than blockers for the Lotus-owned boundary WTBD.
...
19. this boundary-hardening slice adds `client_communication` as a deferred source-event family
    and `DPM_PORTFOLIO_MEMORY_CLIENT_COMMUNICATION_BOUNDARY` evidence with blocked
    ...
    `ClientCommunicationRecord:v1` source product, and deterministic content hash.

Remaining dependencies before full support claim:

None for the current source-backed portfolio-memory support claim. Future OMS execution and client
communication remain separate source-owner roadmap scopes and are explicitly published as deferred
posture rather than hidden manage support. Persisted PM operating quality score runs are supported
separately; portfolio memory still projects no PM-scoring events until a separate source-event
family is governed.
...
18. Gateway PR #242 (`ff4013f`) downstream portfolio-memory source-lineage search proof:
    `/api/v1/dpm/command-center/portfolio-memory/search` forwards Manage
    `source_system`, `source_type`, `source_scan_limit`, paging, and support-boundary posture
    without reconstructing memory facts or querying source-owner stores,
19. Workbench PR #350 (`4e515a3`) downstream portfolio-memory source-lineage rendering proof:
    Gateway-only portfolio-memory search consumption renders represented source-family posture,
    facets, applied filters, and support boundaries without global portfolio-universe discovery or
    broader cross-app source-event search,
20. Manage report-input context tests:
...
21. `lotus-ai` PR #62,
22. `lotus-ai` wiki publication commit `5267759`,
23. `lotus-ai` post-merge focused test proof:
...
24. `lotus-ai` PR #64 merge commit `7d73564`,
25. `lotus-ai` wiki publication commit `a4e70d3`,
26. `lotus-ai` focused source-event proof:
...
27. `lotus-ai` PR-grade local proof:
    `make check`,
28. `lotus-archive` PR #25 merge commit `aa3a3a8f28b666cb85100c0859f77ff2dab9cede`,
29. `lotus-archive.wiki` publication commit `d5e5918`,
30. `lotus-archive` local and GitHub proof:
...
31. manage source-event posture proof:
    `python -m pytest tests/unit/dpm/api/test_portfolio_memory_api.py -q`,
32. manage API governance proof:
    `make openapi-gate`, `make api-vocabulary-gate`.

Promotion proof still required:

1. source-owner tests, API documentation, and canonical proof for any future OMS execution,
   client-communication, global portfolio-universe discovery, broader cross-app source-event
   search, or richer PM-scoring product before those separate deferred families can move to
   `SUPPORTED`.
...
| RFC41-WTBD-003 | Tactical house-view, risk-event, and campaign/bulk-review cohorts | CIO/risk/campaign source owners, with `lotus-risk` owning the first risk-event cohort source product, `lotus-advise` owning the first tactical house-view cohort source product, `lotus-manage` owning the first campaign membership and discovery envelope, `lotus-gateway` owning campaign-definition BFF composition, and `lotus-workbench` owning bounded campaign-definition rendering | Completed for Lotus-owned supportability: `lotus-risk` `RiskEventAffectedCohort:v1` is merged, CI-proven, wiki-published, and mirrored in platform mesh governance; `lotus-manage` consumes it for `RISK_EVENT` preview/create. `lotus-advise` `TacticalHouseViewAffectedCohort:v1` is merged, CI-proven, and wiki-published as source-owned affected-cohort evidence; `lotus-manage` consumes it for bounded `TACTICAL_HOUSE_VIEW` preview/create. `lotus-manage` now also supports bounded `BULK_REVIEW_CAMPAIGN` preview/create through `BulkReviewCampaignMembership:v1` over source-backed candidate portfolios with optional approval/expiry/actor-entitlement governance evidence, plus persisted `BulkReviewCampaignDiscovery:v1` summaries at `GET /api/v1/rebalance/waves/campaign-discovery` with hashed `BulkReviewCampaignUniversePosture:v1` deferred-owner, required future `GlobalPortfolioUniverseCampaignCandidateSet:v1`, blocked bank-wide candidate-discovery capability, and content-hash evidence, bounded campaign operating queue at `GET /api/v1/rebalance/waves/campaign-operating-queue`, bounded read-only campaign approval-attention inbox at `GET /api/v1/rebalance/waves/campaign-approval-inbox`, bounded read-only workflow board at `GET /api/v1/rebalance/waves/campaign-workflow-board`, bounded read-only assignment plan at `GET /api/v1/rebalance/waves/campaign-assignment-plan`, bounded read-only workflow automation readiness at `GET /api/v1/rebalance/waves/campaign-workflow-automation` with hashed `BulkReviewCampaignWorkflowCapabilityPosture:v1` naming blocked external workflow task creation, assignment, synchronization, escalation, and completion capabilities, required future `ExternalWorkflowOrchestrationRecord:v1` source ownership, and promotion requirements for source ownership, source-product contracts, lineage/freshness, consumer declaration, Gateway/Workbench realization, and audit/reconciliation evidence, fail-closed campaign-definition preview readiness at `GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/preview-readiness`, bounded campaign-definition launch packages at `GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/launch-package`, deterministic durable launch at `POST /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/launch`, paged launch-history audit evidence at `GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/launch-history`, append-only campaign approval-decision evidence at `POST`/`GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/approval-decisions`, append-only assignment-action evidence, controlled assignment-task lifecycle evidence at `POST`/`GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/assignment-tasks` plus transition evidence at `POST /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/assignment-tasks/{task_ref}/transitions`, and append-only maker-checker control evidence at `POST`/`GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/maker-checker-controls`. `lotus-gateway` PR #212 exposes campaign-definition list/get/upsert BFF routes under `/api/v1/dpm/command-center/waves/campaign-definitions*`; PR #231 (`ea6c036`, Main Releasability Gate `25989936539`) extends bounded lifecycle-events, launch-history, launch-package, durable launch, and campaign-discovery BFF preservation without local cohort, readiness, order, or OMS calculation. `lotus-workbench` PR #184 renders the active campaign-definition list through Gateway/BFF; PR #244 (`31ea877`, Main Releasability Gate `25989936388`) validates Gateway-only READY-gated launch and paged launch-history/empty-state/no-order/no-OMS boundary rendering without browser-side cohort, membership, readiness, or execution calculation. | Global portfolio-universe campaign discovery remains a deferred source-owner promotion dependency and external workflow orchestration remains an external-owner promotion dependency, both represented as unsupported boundary postures rather than open Lotus-side implementation gaps. |
...
| RFC41-WTBD-010 | External execution integration | Future execution/OMS owner or governed operations integration | Completed for Lotus-owned boundary supportability; external execution/OMS remains an external-owner promotion dependency | RFC-0041 intentionally stops at internal operations handoff evidence, preserves `external_execution_claimed=false`, and now emits fail-closed wave proof-pack and report-input boundary evidence for unsupported execution/OMS scope. Lotus does not claim order routing, acknowledgements, fills, settlement, reconciliation, or execution. |
...
and mirrored into platform mesh governance through `lotus-platform` PR #313 (`4218d4319d5dac82e87106429fadb14247c36515`).
...
The broader WTBD is closed for Lotus-owned supportability because risk-event, tactical
house-view, bounded campaign membership, persisted campaign discovery, controlled assignment-task
readiness, append-only campaign workflow ledgers, maker-checker controls, portfolio-memory
projection, and downstream Gateway/Workbench realization are implemented and governed.
Global portfolio-universe campaign discovery remains a deferred source-owner promotion dependency,
and external workflow orchestration remains an external-owner promotion dependency rather than an
open Manage implementation gap. `lotus-manage` now exposes bounded
...
membership, readiness, or execution calculation. Future campaign depth should promote global
universe and external workflow products only after source-owner contracts, lineage, reconciliation,
Gateway/Workbench realization, and audit evidence are proven.
...
board over the existing read models. Later assignment-task, workflow-automation, universe-boundary,
maker-checker, portfolio-memory, and downstream realization slices close RFC41-WTBD-003 for
Lotus-owned supportability; global portfolio-universe discovery and external workflow
orchestration remain explicit deferred owner dependencies and unsupported claims.
...
operator read model with explicit PM, operations, and governance escalation tiers. Later controlled
task lifecycle, workflow-automation, universe-boundary, maker-checker, portfolio-memory, and
downstream realization slices close RFC41-WTBD-003 for Lotus-owned supportability; global
portfolio-universe discovery and external workflow orchestration remain explicit deferred owner
dependencies and unsupported claims.
...
universe, recalculate membership, or claim OMS execution. RFC41-WTBD-003 is closed for Lotus-owned supportability;
global portfolio-universe campaign discovery and broader workflow automation beyond controlled
Manage-side assignment tasks remain explicit deferred owner dependencies and unsupported claims.

2026-05-20 campaign universe-boundary posture addendum:

`BulkReviewCampaignUniversePosture:v1` now includes a deterministic content hash,
`global_portfolio_universe_owner_posture=DEFERRED_SOURCE_OWNER`, required future
`GlobalPortfolioUniverseCampaignCandidateSet:v1`, and blocked capability codes for bank-wide
portfolio-universe scans, candidate discovery, candidate eligibility calculation, source-fact
recalculation, and membership recomputation. The posture is emitted on campaign-discovery rows and
propagates through queue/approval/workflow/assignment read models that embed discovery rows. This
hardens RFC41-WTBD-003 by making the global portfolio-universe gap machine-readable rather than a
...
recalculate membership, or claim OMS execution. The capability posture now also carries
machine-readable `promotion_requirements` for certified external workflow source ownership,
`ExternalWorkflowOrchestrationRecord:v1`, source-product contract publication, producer lineage and
freshness controls, Manage consumer declaration, Gateway/Workbench realization, and external
workflow audit/reconciliation evidence before any external orchestration can be promoted.
RFC41-WTBD-003 is closed for Lotus-owned supportability; global portfolio-universe campaign discovery and any external
workflow orchestration beyond Manage-side task readiness and append-only evidence ledgers remain explicit deferred owner dependencies and unsupported claims.
...
recalculate membership, or claim OMS execution. RFC41-WTBD-003 is closed for Lotus-owned supportability; global
...
`BULK_REVIEW_CAMPAIGN_ASSIGNMENT_TASK_TRANSITION`, and
`BULK_REVIEW_CAMPAIGN_MAKER_CHECKER_CONTROL` events with stable event identity, source refs,
content hashes, supportability posture, and bounded metadata. Transition events preserve
transition identity, status movement, SLA posture, source refs, and task linkage without raw
transition rationale. They deliberately do not copy raw campaign payloads, discover the global
...
Lotus-owned closure boundary:

RFC-0041 intentionally ends at internal operations handoff evidence. Lotus-owned supportability is
complete for the bounded boundary claim because Manage preserves no-execution posture and emits
fail-closed boundary evidence. No external execution owner or OMS contract is established, and
claiming execution would overstate manage's current business capability.
...
Completed for Lotus-owned boundary supportability. Remaining execution/OMS work is an
external-owner promotion dependency, not open Lotus-app implementation scope. Lotus does not claim
order routing, acknowledgement, fill, settlement, reconciliation, or execution.
...
| RFC42-WTBD-006 | Source-owned realized risk/performance/tax/FX/cash/liquidity outcome methodologies | `lotus-risk`, `lotus-performance`, `lotus-core`, future source owners | Done for Lotus-owned source-methodology supportability; external/non-claim boundaries remain unsupported | RiskMetricsReport, drawdown analytics maximum drawdown, average drawdown, ulcer index, and time under water, concentration response position HHI, top-position weight, top-N cumulative weight, issuer HHI, top issuer weight, and selected measures, rolling metric summaries, historical attribution selected measures, performance RFC-046 TWR daily evidence/supportability/benchmark posture, workspace-summary TWR, active return, stateful MWR, contribution selected measures, attribution selected measures, source-owned portfolio-level `currency_attribution_totals`, core HoldingsAsOf cash totals, core TransactionLedgerWindow explicit transaction-row measures and field-aware book/trade reporting-currency restatement, core PortfolioCashflowProjection total/booked/projected-settlement cashflow, core PortfolioLiquidityLadder operational bucket measures, core `ClientTaxProfile:v1` / `ClientTaxRuleSet:v1` profile/rule evidence, core `ClientIncomeNeedsSchedule:v1` / `LiquidityReserveRequirement:v1` / optional `PlannedWithdrawalSchedule:v1` bounded reference evidence, core `PortfolioRealizedTaxSummary:v1` portfolio-level explicit realized tax evidence, and core `PortfolioCashMovementSummary:v1` signed cash movement bucket evidence now have source-owned implementation truth. Manage outcome-review search now exposes bounded source-owner/source-type filters and facets over persisted review lineage so consumers can discover which source families are represented without querying source-owner stores or recalculating realized source truth. Gateway PR #242 forwards those filters and preserves Manage-owned facets/support boundaries; Workbench PR #350 renders Gateway-only applied filters, source-family facets, and support-boundary posture. Broader bank/external capabilities beyond Lotus-owned methodology supportability remain unsupported non-claims: tax advice, tax-loss harvesting suitability, after-tax optimization, jurisdiction-specific recommendation, client-tax approval, tax-reporting certification, financial-planning advice, funding recommendation, treasury instruction, predictive execution, best-execution, venue routing, order routing, OMS acknowledgement, fills, settlement, and reconciliation. |
| RFC42-WTBD-007 | External execution/OMS integration and acknowledgements | Execution/OMS owner, `lotus-core` source-boundary posture, `lotus-manage` consumer | Done for Lotus-owned boundary supportability; bank-owned OMS ingestion remains an external-owner promotion dependency | `lotus-core` `ExternalOrderExecutionAcknowledgement:v1` now exposes a fail-closed `UNAVAILABLE` source-product posture and Manage preserves it as construction authority diagnostics and RFC-0042 realized outcome execution-quality evidence with acknowledgement counts, empty acknowledgement rows, missing data families, blocked capabilities, lineage, and source hashes. Outcome-review supportability, report input, and AI evidence input now expose structured `DPM_OUTCOME_EXTERNAL_EXECUTION_BOUNDARY` evidence with blocked capabilities, required future execution/OMS owner, required `ExternalOrderExecutionAcknowledgement:v1` source product, execution-quality dimension posture, acknowledgement-count posture, deterministic content hash, and promotion requirements for certified OMS source ownership, source-product contracts, lineage/freshness, acknowledgement/fill/settlement reconciliation, Manage consumer declaration, Gateway/Workbench realization, and operations audit evidence. RFC-0042 can compare expected and realized non-execution evidence while marking execution-quality evidence blocked, but production OMS integration still needs a bank-owned execution/OMS owner, ingestion controls, acknowledgements, fills, settlement, and reconciliation contract before support can be promoted. |
| RFC42-WTBD-008 | PM operating quality framework / configurable scoring | `lotus-manage` with source evidence from `lotus-core`, `lotus-risk`, and `lotus-performance`; Gateway composition; `lotus-ai` narrative only; Workbench through Gateway only | Completed for the bounded Manage backend and first-wave product support claim: policy administration, score-run preview, immutable create/read/list score-run lifecycle, optional source-owned PM-book materialization, bank approval evidence, fairness-review evidence, expiry controls, actor entitlement checks, bank-defined peer-group/lookback-window scope materialization, bounded source-segment fairness-analysis preview/create/read/list, bounded immutable review-action preview/create/read/list, bounded immutable support-summary invocation preview/create/read/list history, bounded portfolio-memory score-run, review-action, and summary-invocation lineage projection, Gateway policy/score-run/fairness-analysis/support-summary/review-action BFF composition, AI-owned support-only PM quality summary, and Gateway-only Workbench policy/score-run/fairness-analysis/support-summary plus review-action ledger/detail and preview-before-create command UX are implemented, merged, validated, and wiki-published where changed. | `lotus-manage` now owns configurable, evidence-first, default-disabled PM operating quality policies at `PUT /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `GET /api/v1/rebalance/pm-operating-quality/policies`, and `GET /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`; score-run preview at `POST /api/v1/rebalance/pm-operating-quality/score-runs/preview`; persisted score-run lifecycle at `POST /api/v1/rebalance/pm-operating-quality/score-runs`, `GET /api/v1/rebalance/pm-operating-quality/score-runs`, and `GET /api/v1/rebalance/pm-operating-quality/score-runs/{score_run_id}`; source-segment fairness-analysis preview/create/list/get at `/api/v1/rebalance/pm-operating-quality/fairness-analyses*`; review-action preview/create/list/get at `/api/v1/rebalance/pm-operating-quality/review-actions*`; review-gated support-summary invocation preview/create/list/get at `/api/v1/rebalance/pm-operating-quality/summary-invocations*`; portfolio-memory `PM_QUALITY_SCORE_RUN` lineage events for persisted score runs whose source-owned Core PM-book membership evidence includes the requested portfolio; portfolio-memory `PM_QUALITY_REVIEW_ACTION` supervisory events for review actions over those score runs without raw rationale, score values, PM ranking, client-contact, trade, order, OMS, or execution claims; and portfolio-memory `PM_QUALITY_SUMMARY_INVOCATION` workflow-lineage events for support-summary invocations over those score runs without generated summary text, prompt bodies, model responses, downstream summary UX, PM ranking, client-contact, trade, order, OMS, or execution claims. `PmOperatingQualitySummaryInvocation:v1` records only score-run/review-action identity plus state-specific workflow, completed artifact, hash, or bounded failure evidence; it does not store generated AI narrative text, expose raw review rationale, recalculate scores, recompute fairness, rank PMs, contact clients, approve trades, route orders, or claim OMS execution. `lotus-gateway` PR #213 (`62ce4c4`, wiki `a4c9db9`) exposes bounded policy, score-run, fairness-analysis, and score-run AI-summary handoff routes, and `lotus-gateway` PR #234 (`c74b3e1514e8875d1cf7a479280e9deddd51a8db`, Main Releasability Gate `26163382026`) exposes bounded PM-quality review-action preview/create/list/get routes, preserving Manage truth without calculating scores, ranking PMs, administering policy locally, or creating HR, compensation, conduct, approval, client-contact, execution, or OMS decisions. `lotus-workbench` PR #245 (`2af063b`, wiki `2ba368d`, Main Releasability Gate `25991445845`) completes the Gateway-only PM operating quality product surface for policy and score-run evidence, fairness-analysis preview/create/list/detail, persisted-analysis readback, supportability/reason/source-ref/forbidden-use rendering, and review-gated PM-quality support-summary invocation; `lotus-workbench` PR #302 (`8052d92`) and PR #303 (`3581b04`) add Gateway-only read-only PM-quality review-action ledger/detail rendering and component-level boundary tests. Workbench PR #314 (`6bd43c8`) adds Gateway-only review-action preview-before-create command UX, PR #315 (`806193f`) extracts the bounded command-control component, PR #316 (`fdc37b5`) extracts the PM-quality API module, and latest Workbench Main Releasability Gate `26206758251` is green at `1dfdeb14a24c1df25a6d4676c6d6c6506619efb3`. `lotus-ai` PR #70 (`1951f62`, wiki `038a1a1`) adds `pm_quality_summary.pack@v1` for review-gated support-only summaries over Manage-owned `PmOperatingQualityScoreRun` evidence; it validates score-run identity, source refs, supportability posture, optional bounded portfolio-memory context, and forbidden-use controls without calculating scores, ranking PMs, generating HR/compensation/conduct decisions, contacting clients, approving trades, routing orders, claiming execution, or inventing missing facts. Optional `pm_book_scope` resolves lotus-core `PortfolioManagerBookMembership:v1`, records explicit `book_scope_evidence`, and fails closed for unavailable, incomplete, degraded, or empty membership. Enabled policies require bank-defined dimensions, weights, thresholds, peer groups, governance approval, fairness-review evidence, source refs, and non-use posture; `peer_group_policy` and `lookback_window_policy` materialize into score-run `scope_evidence`, preserve peer-group/lookback source refs in the content hash, and fail closed for dated evidence outside the approved lookback window without Manage discovering peers, ranking PMs, or owning source methodology. Missing evidence, expired governance, unauthorized actors, and prohibited HR, compensation, conduct-enforcement, autonomous-ranking, and AI-generated scoring use are rejected. Fairness analysis consumes persisted score-run ids and source-defined operating segments, validates common policy/as-of scope, minimum scorable segment counts, and governed average-score spread, and does not infer protected classes or create PM rankings. Approval workflow beyond immutable review actions and external OMS/execution remain future product depth rather than part of this support claim. |

### Detailed Follow-Up Items

Historical WTBD-006 source-owner addenda below retain their at-the-time "advances but does not
close" wording where they described incremental source-family slices. The 2026-05-24
RFC42-WTBD-006 closure addendum above supersedes that classification for Lotus-owned
source-methodology supportability while preserving unsupported advice, predictive execution, and
OMS non-claims.
...
`POST /api/v1/rebalance/pm-operating-quality/summary-invocations`, lists bounded pages at
`GET /api/v1/rebalance/pm-operating-quality/summary-invocations`, and retrieves one history row at
`GET /api/v1/rebalance/pm-operating-quality/summary-invocations/{summary_invocation_id}`. The
implementation uses `src/core/pm_quality/summary_history.py` plus the
`0014_pm_quality_summary_invocations.sql` migration to preserve score-run identity, review-action
gate identity, target content hashes, workflow handoff, completed artifact refs and hashes, or failed invocation reason evidence, requested
actor, reason codes, and source refs. Stored reads do not store generated AI narrative text, expose
raw review rationale, recalculate scores, recompute fairness, rank PMs, create HR, compensation,
conduct, client-contact, trade, order, OMS, or execution decisions, or claim a broader approval
workflow beyond immutable review evidence.
...
2026-05-21 PM-quality portfolio-memory summary-invocation projection addendum:

Portfolio memory now projects bounded `PM_QUALITY_SUMMARY_INVOCATION` events for immutable PM
operating quality summary invocations whose summarized score-run has source-owned Core PM-book
membership evidence for the requested portfolio. The projection preserves summary-invocation
identity, score-run and review-action identity, target content hashes, workflow-run and artifact
refs, optional artifact hash, source refs, content hash, invocation state, and
`PM_QUALITY_SUMMARY_TEXT_BOUNDARY` posture while deliberately omitting generated summary text,
prompt bodies, model responses, raw review rationale, and score values. It does not recalculate
scores, recompute fairness, rank PMs, create HR/compensation/conduct outcomes, contact clients,
approve trades, generate orders, route execution, claim OMS acknowledgement, project downstream
summary UX, or discover the global portfolio universe.
...
methodology, approval workflow beyond immutable review actions, and any future
...
Why it is only boundary-supported now:
...
RFC42-WTBD-007 is complete for Lotus-owned supportability because Lotus has implemented the
fail-closed source contract, Manage consumer preservation, supportability/report/AI boundary
evidence, lineage, deterministic hashes, promotion requirements, and explicit no-order/no-OMS
non-claims. A bank-owned OMS owner must still certify ingestion, acknowledgements, fills,
settlement, and reconciliation before the blocked boundary can be promoted to ready execution
support.
...
deterministic content hash. It now also names promotion requirements for certified OMS source
ownership, source-product contract publication, producer lineage/freshness, acknowledgement/fill/
settlement reconciliation, Manage consumer declaration, Gateway/Workbench realization, and
operations audit evidence. This gives Gateway, Workbench, report, AI, support, and operations
...
carries a deterministic content hash. It now also names promotion requirements for certified
client-communication source ownership, source-product contract publication, lineage/freshness,
delivery/approval/audit reconciliation, Manage consumer declaration, Gateway/Workbench
realization, and consent/evidence controls. This gives Gateway, Workbench, report, AI, support, and
...
1. approval workflow beyond immutable review actions,
