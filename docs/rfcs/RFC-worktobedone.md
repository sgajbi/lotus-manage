...
| RFC-0039 | RFC39-WTBD-001 through RFC39-WTBD-004, RFC39-WTBD-006, RFC39-WTBD-010, and the bounded first-wave RFC39-WTBD-007 and RFC39-WTBD-009 results are incorporated into `docs/rfcs/RFC-0039-advanced-portfolio-construction-and-rebalance-alternatives.md`. | RFC39-WTBD-005 and runtime RFC39-WTBD-008 remain broader risk/performance and treasury-depth controls. RFC39-WTBD-008 now has external treasury source-product contract boundaries, active fail-closed hedge-readiness and currency-exposure posture merged in `lotus-core`, platform mirror truth, and first Manage consumption of the unavailable hedge-readiness posture; external treasury ingestion and Manage currency-exposure consumption remain pending. |
...
| RFC40-WTBD-009 | Completed for manage proof-pack preservation of selected-alternative `RegimeScenarioPackEvaluation:v1` context and generation-time direct proof-pack enrichment from caller-supplied source-owned regime scenario context when the selected alternative does not already carry it; `lotus-risk` now also owns optional reconciled per-security scenario contribution rows in the source product. | CIO approval workflow evidence, effective-period exception posture, portfolio/mandate applicability evidence, and richer scenario methodology remain future source-owner / product-depth work. |
| RFC41-WTBD-003 | `lotus-risk` owns `RiskEventAffectedCohort:v1` at `POST /analytics/risk/risk-event-cohorts/evaluate`, merged in `lotus-risk` PR #115 (`bd69d1576d8c01bdcfd2309202ef37f780cc2d06`) and published to `lotus-risk.wiki` commit `91f933a`; `lotus-platform` PR #313 (`4218d4319d5dac82e87106429fadb14247c36515`) mirrors the product declaration for mesh governance. `lotus-manage` consumes that product for bounded `RISK_EVENT` wave preview/create from caller-supplied candidate portfolios with source-supplied exposure weights, fail-closed dependency handling, source-ref preservation, OpenAPI/API-vocabulary updates, and repo-native consumer declaration coverage. `lotus-advise` owns `TacticalHouseViewAffectedCohort:v1`; `lotus-manage` consumes it for bounded `TACTICAL_HOUSE_VIEW` wave preview/create over managed/discretionary source-backed candidates, preserving Advise cohort refs and candidate source refs without recomputing house-view or portfolio facts. `lotus-manage` now also owns `BulkReviewCampaignMembership:v1` for bounded `BULK_REVIEW_CAMPAIGN` wave preview/create over source-backed candidate portfolios with source-owned portfolio types, DPM portfolio-type filtering, deterministic membership refs and hash, optional approval/expiry/actor-entitlement governance evidence, persisted `BulkReviewCampaignDefinition:v1` definitions, persisted `BulkReviewCampaignDiscovery:v1` summaries at `GET /api/v1/rebalance/waves/campaign-discovery`, campaign-definition retirement at `POST /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/retire`, campaign-definition supersession at `POST /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/supersede`, lifecycle-event projection at `GET /api/v1/rebalance/waves/campaign-definitions/{campaign_id}/versions/{campaign_version}/lifecycle-events`, and fail-closed validation. `lotus-gateway` PR #212 (`891d00dea0328525ada14fabad278af1d1b9386b`) now exposes campaign-definition list/get/upsert BFF routes and published Gateway wiki commit `f49e84f`, preserving Manage campaign payloads without recomputing cohorts or membership. `lotus-workbench` PR #184 (`502aa2113a6e3954d53a35f9f956d258c84d5be0`) renders the active campaign-definition list and published Workbench wiki commit `694d9cd`. | Broader campaign workflow surfaces beyond definition lifecycle controls are not yet promoted. |
...
| 2 | RFC41-WTBD-003 - Tactical house-view, risk-event, and campaign/bulk-review cohorts | Moves the rebalance wave operating model toward bank operating workflows without inventing source-owned cohorts. Risk-event source ownership, bounded manage consumption, Advise-owned tactical house-view source ownership, bounded Manage tactical house-view consumption, first-wave Manage-owned bulk-review campaign membership with optional governance evidence, immutable Manage-owned campaign definitions over source-backed candidate sets, Gateway campaign-definition BFF composition, and Workbench active campaign-definition list rendering are now implemented. | Global discovery and broader campaign workflow surfaces are added only after source behavior and product evidence are proven. |
...
| RFC39-WTBD-008 | External treasury/currency overlay source boundary | `lotus-core` ingestion of external bank treasury/currency products, consumed by `lotus-manage` | Planned external-source contracts plus active fail-closed hedge-readiness posture merged in `lotus-core`; Manage consumes the unavailable posture; runtime external treasury ingestion remains pending | Current support uses FX readiness and bounded currency-overlay context. Lotus will not introduce a `lotus-treasury` app. `lotus-core` PR #365 (`c7fa07b0`, wiki `067f919`) declares planned contracts for external currency exposure, hedge policy, FX forward curves, and eligible hedge instruments. `lotus-core` PR #366 (`9e86df3b`, wiki `617e4e6`) exposes `ExternalHedgeExecutionReadiness:v1` as an active fail-closed `UNAVAILABLE` route. Manage now consumes that route through stateful core sourcing and preserves missing external treasury data families, blocked capabilities, lineage, source hash, and reason codes in currency-overlay diagnostics so hedge realization remains blocked while ingestion is unavailable. Manage still must not price forwards, approve hedge policy, choose counterparties, claim best execution, produce OMS acknowledgements, claim fills/settlement, or perform autonomous treasury action. |
...
artifacts. Manage has not yet consumed `ExternalCurrencyExposure:v1`; that remains the next
bounded backend adapter slice before any Gateway or Workbench realization.

Manage now consumes that fail-closed posture through stateful core sourcing: the core resolver posts
to `/integration/portfolios/{portfolio_id}/external-hedge-execution-readiness`, derives exposure
currencies from the resolved source context, and attaches `ExternalHedgeExecutionReadiness:v1` to
the DPM execution context. Construction enrichment lifts the posture into
`currency_overlay_context` with source refs, content hash, missing external treasury data families,
blocked capabilities, and reason codes. `UNAVAILABLE` maps to `BLOCKED`, so `CURRENCY_OVERLAY`
alternatives preserve a fail-closed external treasury readiness diagnostic instead of deriving a
local hedge-readiness claim.

This advances RFC39-WTBD-008 from source-owner fail-closed runtime posture to first Manage
consumption of that posture. It does not certify an external treasury ingestion table or add hedge
advice, forward pricing, treasury instruction, counterparty selection, best execution, OMS
...
scenario pack id, worst-case loss, maximum allowed loss, supportability state, and bounded reason
codes without manage-local scenario methodology. The `lotus-risk` source product now also supports
...
path. It also does not yet receive CIO approval workflow evidence, effective-period exception
posture, or portfolio/mandate applicability evidence beyond the first-wave evaluation reason codes
and optional risk-owned contribution rows.
...
1. CIO approval and applicability evidence from `lotus-risk` / CIO authority if product claims
   require it,
2. proof-pack section-state behavior for stale, missing, inapplicable, and contribution-partial
   scenario evidence,
...
The first-wave source, selected-alternative preservation path, and generation-time direct
proof-pack enrichment path are implemented and audited. Richer CIO approval/applicability evidence
should be handled only if a future proof-pack/product RFC requires those fields and the source owner
exposes the required product.
...
| What was proven through testing and evidence | Focused proof-pack tests passed and now assert source refs, source hashes, bounded scenario facts, scenario metrics, reason codes, direct source-context enrichment, degraded fallback behavior, and malformed-context rejection. `lotus-risk` PR #116 passed Feature Lane and PR Merge Gate checks, local live API proof for contribution rows, negative 422 validation for unreconciled components, risk log review, and canonical Workbench validation for `PB_SG_GLOBAL_BAL_001`. |
| Expected-standard decision | The manage-owned part of RFC40-WTBD-009 reaches the expected standard for selected-alternative evidence preservation and direct generation-time source-context enrichment, and the risk-owned scenario contribution source product reaches the expected standard for optional reconciled contribution rows. CIO approval workflow, effective-period exception posture, and portfolio/mandate applicability remain future source-owner/product depth. |
...
`/api/v1/dpm/command-center/waves/campaign-definitions*`, preserving campaign payloads without
recomputing cohort facts, campaign membership, portfolio eligibility, or maker-checker posture.
`lotus-workbench` PR #184 renders the active campaign-definition list through Gateway/BFF without
browser-side cohort or membership calculation. Future campaign depth should expand the now-promoted
...
6. Workbench active campaign-definition list consumes Gateway campaign-definition BFF routes only,
...
lifecycle controls, bounded lifecycle-event projection, bounded persisted campaign discovery, and
Gateway BFF composition for campaign-definition list/get/upsert plus Workbench active list
...
| Expected-standard decision | This reaches the backend standard for bounded governance evidence on Manage-owned campaign membership. Tactical house-view is supported separately through the Advise source product, persisted campaign definitions are supported in Manage, Gateway campaign-definition BFF composition is supported through `lotus-gateway` PR #212, and Workbench active campaign-definition list rendering is supported through `lotus-workbench` PR #184. Global campaign discovery, broader campaign workflow surfaces, and OMS execution remain outside the support claim. |
...
| RFC42-WTBD-008 | PM operating quality framework / configurable scoring | `lotus-manage` with source evidence from `lotus-core`, `lotus-risk`, and `lotus-performance`; Gateway composition; `lotus-ai` narrative only | First bounded policy administration, score-run preview, immutable create/read/list score-run lifecycle, optional source-owned PM-book materialization, bank approval evidence, fairness-review evidence, expiry controls, actor entitlement checks, bank-defined peer-group/lookback-window scope materialization, bounded source-segment fairness-analysis preview, bounded portfolio-memory score-run lineage projection, Gateway BFF composition, and AI-owned support-only PM quality summary are implemented; Workbench PM-quality UI and Gateway/Workbench fairness-analysis product realization remain future scope | `lotus-manage` now owns configurable, evidence-first, default-disabled PM operating quality policies at `PUT /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`, `GET /api/v1/rebalance/pm-operating-quality/policies`, and `GET /api/v1/rebalance/pm-operating-quality/policies/{policy_id}/versions/{policy_version}`; score-run preview at `POST /api/v1/rebalance/pm-operating-quality/score-runs/preview`; persisted score-run lifecycle at `POST /api/v1/rebalance/pm-operating-quality/score-runs`, `GET /api/v1/rebalance/pm-operating-quality/score-runs`, and `GET /api/v1/rebalance/pm-operating-quality/score-runs/{score_run_id}`; source-segment fairness-analysis preview at `POST /api/v1/rebalance/pm-operating-quality/fairness-analyses/preview`; and portfolio-memory `PM_QUALITY_SCORE_RUN` lineage events for persisted score runs whose source-owned Core PM-book membership evidence includes the requested portfolio. `lotus-gateway` PR #213 (`62ce4c4`) exposes the bounded `/api/v1/dpm/command-center/pm-operating-quality/*` BFF route family and published wiki source at `a4c9db9`, preserving Manage policy and score-run truth without calculating scores, ranking PMs, administering policy locally, or creating HR, compensation, conduct, approval, client-contact, execution, or OMS decisions. `lotus-ai` PR #70 (`1951f62`, wiki `038a1a1`) adds `pm_quality_summary.pack@v1` for review-gated support-only summaries over Manage-owned `PmOperatingQualityScoreRun` evidence; it validates score-run identity, source refs, supportability posture, optional bounded portfolio-memory context, and forbidden-use controls without calculating scores, ranking PMs, generating HR/compensation/conduct decisions, contacting clients, approving trades, routing orders, claiming execution, or inventing missing facts. Optional `pm_book_scope` resolves lotus-core `PortfolioManagerBookMembership:v1`, records explicit `book_scope_evidence`, and fails closed for unavailable, incomplete, degraded, or empty membership. Enabled policies require bank-defined dimensions, weights, thresholds, peer groups, governance approval, fairness-review evidence, source refs, and non-use posture; `peer_group_policy` and `lookback_window_policy` materialize into score-run `scope_evidence`, preserve peer-group/lookback source refs in the content hash, and fail closed for dated evidence outside the approved lookback window without Manage discovering peers, ranking PMs, or owning source methodology. Missing evidence, expired governance, unauthorized actors, and prohibited HR, compensation, conduct-enforcement, autonomous-ranking, and AI-generated scoring use are rejected. Fairness-analysis preview consumes persisted score-run ids and source-defined operating segments, validates common policy/as-of scope, minimum scorable segment counts, and governed average-score spread, and does not infer protected classes or create PM rankings. `lotus-ai` may summarize bounded evidence but must not calculate or own scores. |
...
Gateway/Workbench product invocation remains future owner scope and must preserve Manage score-run
truth without local scoring or prompt construction.
...
3. Gateway/Workbench product invocation for `pm_quality_summary.pack@v1`,
...
Gate passed, and Gateway wiki source was published at `a4c9db9` with zero drift. Workbench
PM-quality UI remains future owner scope and must consume this Gateway BFF only.
...
score ownership. Workbench UI, Gateway/Workbench fairness-analysis product realization, and
Gateway/Workbench PM-quality summary invocation remain future expansion.
