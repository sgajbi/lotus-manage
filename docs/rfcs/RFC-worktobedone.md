...
| RFC-0038 | RFC38-WTBD-001 through RFC38-WTBD-006 and RFC38-WTBD-008 are incorporated into `docs/rfcs/RFC-0038-mandate-digital-twin-health-and-command-center.md`. | RFC38-WTBD-007 is partial for Manage-side source-analytics posture and bounded supplied-signal preservation; broader source-owner risk/performance health products remain future. |
...
59 total WTBD items: 47 done on merged/published Lotus-owned truth, 8 partial or in progress, and
4 remaining/open. The 2026-05-22 Lotus-owned completion recount moves RFC39-WTBD-008 and
RFC42-WTBD-007 out of partial because Lotus has implemented the fail-closed external treasury and
...
The RFC38-WTBD-007 mandate-health risk/performance posture slice moves that row from open to
partial: mandate-health snapshots now emit typed `source_analytics_posture` evidence that
preserves supplied tracking-error and performance-attention signals while naming the future
source-owned risk/performance health products and blocked local-calculation capabilities.
...
| RFC38-WTBD-007 | Broader risk and performance health enrichment | `lotus-risk`, `lotus-performance`, consumed by `lotus-manage` | Partial: Manage preserves supplied tracking-error/performance-attention signals and emits typed source-analytics posture; broader source-owner health products remain future | Manage health snapshots now carry `source_analytics_posture` naming `MandateRiskHealthContext:v1` and `MandatePerformanceHealthContext:v1`, supplied-signal booleans, preservation posture, blocked local-calculation capabilities, and source-owned methodology reason codes. Manage health still cannot clone risk or performance methodology. Risk/performance attention must come from certified owners. |
...
risk/performance health context. The posture records whether tracking-error and
performance-attention signals were supplied, preserves those signals in the existing `RISK_DRIFT`
and `PERFORMANCE_ATTENTION` dimensions, names future `lotus-risk`
`MandateRiskHealthContext:v1` and `lotus-performance` `MandatePerformanceHealthContext:v1`
products, and lists blocked local-calculation capabilities for tracking error, volatility,
drawdown, attribution, and benchmark-relative performance. This is supportability posture and
source-signal preservation only; Manage does not calculate risk or performance methodology.
