...
| Done on merged/published truth | 42 | Implementation-backed items merged to owning `main` branches, validated, and published where wiki truth changed. |
...
1. source-owner event families for future OMS and PM-scoring events when those products are
   implemented and supportable.
...
2. source-owner tests and canonical proof for each future OMS and PM-scoring event family.
...
owning-app consumer seam for bounded portfolio-memory context, AI now has an owning-app
source-event family over workflow-pack run/review/lineage posture, and archive now has an
owning-app generated-document/client-delivery reissue source-event family. Broader WTBD closure
should wait until OMS and PM-scoring source events and consumers are implemented by their owners
and canonically proven.
