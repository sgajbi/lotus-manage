...
6. unsupported: global campaign discovery, permission-denied, and stale-book cohort semantics until
   owning source products exist,
...
The broader WTBD remains partial because global campaign discovery and broader campaign workflow
surfaces remain future depth. `lotus-gateway` PR #212 now composes the manage-owned
campaign-definition list/get/upsert APIs under
...
manage-owned membership envelope with global discovery and broader workflow behavior only after
...
5. richer campaign cohorts define global discovery governance,
...
governance evidence, immutable persisted campaign definitions, and Gateway BFF composition for
campaign-definition list/get/upsert plus Workbench active list rendering; future slices should add
global discovery and broader campaign workflow surfaces without moving source-fact ownership into
Manage, Gateway, or Workbench.
...
`lotus-gateway` PR #213 without local score calculation or prohibited-use claims. Workbench UI and
Gateway/Workbench fairness-analysis product realization remain future expansion.
