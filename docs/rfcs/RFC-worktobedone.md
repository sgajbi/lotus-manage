...
| RFC40-WTBD-009 | Completed for manage proof-pack preservation of selected-alternative `RegimeScenarioPackEvaluation:v1` context and generation-time direct proof-pack enrichment from caller-supplied source-owned regime scenario context when the selected alternative does not already carry it, including bounded source-supplied CIO approval, effective-period, and portfolio/mandate applicability evidence preservation when present, plus bounded proof-pack state/reason-code posture for missing, stale/effective-period-exception, inapplicable, and contribution-partial source evidence; `lotus-risk` now also owns optional reconciled per-security scenario contribution rows and a v3 auditable scenario-pack methodology in the source product. | Source-owner CIO approval workflow validation, effective-period exception methodology, and portfolio/mandate applicability calculation remain future source-owner / product-depth work. |
...
2. source-owner CIO approval workflow validation and applicability methodology behind stale,
   missing, inapplicable, and contribution-partial governance/applicability evidence,
