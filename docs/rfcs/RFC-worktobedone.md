...
3. the methodology pins booked and projected-inclusive ledger modes, effective as-of date
   ...
   joined row-level transaction-cost and cashflow evidence preservation, optional
...
3. `docs/methodologies/source-data-products/transaction-ledger-window.md` now records the
   `FX_report = FX_local * X_c` methodology, supported field set, validation behavior, and worked
   example for explicit realized FX P&L local evidence,
