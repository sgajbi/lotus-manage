...
6. preserve fail-closed external hedge execution readiness from `lotus-core` when available while
   keeping treasury-depth evidence unavailable until external currency exposure, hedge policy,
   FX forward curves, and eligible hedge instruments are ingested.
...
| Currency-overlay construction | Supported for bounded FX and fail-closed external treasury diagnostics; proposed for ready treasury-depth source support | Current bounded overlay posture is implemented at the manage backend layer. Manage consumes `lotus-core` `ExternalHedgeExecutionReadiness:v1`, `ExternalCurrencyExposure:v1`, `ExternalHedgePolicy:v1`, `ExternalEligibleHedgeInstrument:v1`, and `ExternalFXForwardCurve:v1` when stateful core sourcing is enabled and preserves their unavailable posture as blocked currency-overlay diagnostics. Ready treasury-depth support still requires external bank source contracts and ingestion for currency exposure, hedge policy, FX forward curves, eligible hedge instruments, and ready hedge execution posture. Do not create a `lotus-treasury` app; map deployed bank treasury systems into contracts consumed through core/source-readiness and then manage. |
...
| RFC39-WTBD-009 | `REGIME_STRESS_AWARE` can consume `RegimeScenarioPackEvaluation:v1` from `lotus-risk` when configured; proof packs can also preserve direct generation-time source-owned `regime_stress_context` when selected-alternative scenario context is absent; the source product now also supports optional reconciled per-security scenario contribution rows and v3 scenario/contribution methodology. | CIO approval workflow, portfolio/mandate applicability evidence, and richer product UX remain future depth. |
