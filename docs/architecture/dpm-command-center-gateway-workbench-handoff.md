...
| Governed mandate refresh | `POST /api/v1/mandates/{mandate_id}/refresh-from-core` | Operator automation or Gateway must send the same normalized caller-asserted tenant in required `X-Tenant-Id` and body `tenant_id`. Manage rejects mismatch before Core reads or persistence and forwards the admitted header per Core source request. This is not authenticated-principal proof. |
