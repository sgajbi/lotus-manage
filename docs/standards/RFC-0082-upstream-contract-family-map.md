...
6. `lotus-manage` owns bounded `BulkReviewCampaignMembership:v1` for `BULK_REVIEW_CAMPAIGN`
   ...
   the bounded candidate list for that envelope, but it still does not own relationship
...
| DPM portfolio-universe candidates | `lotus-core` owns bounded mandate-backed candidate discovery, source records, filters, supportability, page completeness, and source-batch fingerprint | Cohort candidate source product | optional `BULK_REVIEW_CAMPAIGN` preview/create candidate resolution from `DpmPortfolioUniverseCandidate:v1` when `campaign_candidate_source=CORE_DPM_PORTFOLIO_UNIVERSE` | manage must fail closed on unavailable, incomplete, degraded, empty, or truncated candidate pages; it must not infer relationship householding, PM ranking, global portfolio-universe ownership, workflow orchestration, client contact, order routing, or OMS execution |
